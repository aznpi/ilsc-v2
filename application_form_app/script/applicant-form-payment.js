...
const checkAUPaymentEligibility = function () {
  let residencevVal = $(
      "select[name=student_address_country] option:selected"
    ).val(),
    nationalityVal = $(
      "select[name=student_nationality] option:selected"
    ).val(),
  ...
  if (countryStudyVal == "Australia") {
    ...
    nationalityEl = disablePaymentCountryList.includes(nationalityVal)
      ? false
      : true;
...
const printPaymentOption = function () {
  let currencyValue =
    ...
    depositAmtValue =
      $("input[name=calculated-deposit-amt]").val() != ""
        ? $("input[name=calculated-deposit-amt]").val()
        : depositAmt;
  countryPaymentEnable =
    $("input[name=program_country_payment_enable]").val() == 1 ? true : false;
  ...
  if (enablePayment && countryPaymentEnable && auPaymentEnable) {
    ...
    $(".application-complete .payment-message-container span#deposit-amt").html(
      currencyValue + " " + depositAmtValue
    );
  } else {
    $(".application-complete .payment-message-container").removeClass(
      "enabled"
    );
  }
};
...
const printPaymentSummary = function () {
  let email = $("input[name=student_email]").val(),
    ...
    middleName =
      $("input[name=student_middle_ename]").val() != ""
        ? " " + $("input[name=student_middle_name]").val()
        : "",
  ...
  (destinationCurrency =
    ...
    (depositTotal =
      $("input[name=calculated-deposit-amt]").val() != ""
        ...
        : depositAmt),
    (paymentMessage =
      "<p>Thank you for enrolling with " +
      programSchoolName +
      '!</p><p>We&apos; ve received your application and will begin reviewing it soon. To ensure priority processing, please submit a deposit payment of <span id="depoist-amt">' +
      currencyToCountryFormatter(depositTotal, destinationCurrency) +
      "</span>. Please note that your enrollment can only be guaranteed once we&apos;ve received full payment as well as confirmed program and housing availability on your chosen start dates. We will be in touch with you to confirm.</p>"),
    (summaryHtml =
      "<table class='payment-summary-tbl'> <tr> <td>Full Name:</td> <td>" +
      fullName +
      "</td> </tr> <tr> <td>Email address:</td> <td>" +
      email +
      "</td> </tr> <tr> <td>Deposit payable to secure your place:</td> <td>" +
      currencyToCountryFormatter(depositTotal, destinationCurrency) +
      "<span class='footnote'>&nbsp;&nbsp;&nbsp;(additional fees might apply depending on the country of method payment)</span></td> </tr> </table>");
...
const addDigitalConfirmation = function () {
  let applicantStatus = $("input[name=online_applicant_status]:checked").val(),
    agentName =
      applicantStatus == "Agent"
        ? $("input[name=agency_contact_person]").val()
        : false,
    agencyName =
      applicantStatus == "Agent" ? $("input[name=agency_name]").val() : false,
    studentName =
    ...
    timeStamp = todayDate.toLocaleTimeString("en-US", {
      hour: "numeric",
      minute: "numeric",
      hour12: true,
    }),
  ...
  digitalConfirmationTxt =
    "Electronic Signature Received: " + signatureName + " " + todayDateStr;
...
const printApplicationSummary = function () {
    ...
    if (studyFormArray[stf].name.includes("dependent")) {
      ...
      for (let pa = 0; pa < programInfoArray.length; pa++) {
        ...
        if (splitName[0] == programInfoArray[pa].inputName) {
          programValue = programInfoArray[pa].inputLabel.includes(
            "Program Type"
          )
            ? studyFormArray[stf].value == 0
              ? "Academic"
              : studyFormArray[stf].value == 2
              ? "Micro-Credentials"
              : studyFormArray[stf].value == 1
              ? "Co-Op"
          ...
          labelValue =
            schoolSelected == "Greystone College" || schoolSelected == "Greystone Institute"
              ? programInfoArray[pa].inputLabel == "Program"
                ? "Program/Course"
                : programInfoArray[pa].inputLabel
              : programInfoArray[pa].inputLabel;
        ...
        if (!programInfoArray[pi].hiddenEnable) {
          if (studyFormArray[stf].name == programInfoArray[pi].inputName) {
            valueStr = studyFormArray[stf].name.includes("program_is_coop_")
              ...
              : studyFormArray[stf].value == 0
              ? "No"
              : studyFormArray[stf].value == "False"
              ? "No"
              : studyFormArray[stf].value == "True"
              ? "Yes"
              : studyFormArray[stf].value;

            if (
              studyFormArray[stf].name.includes(
                "additional_australia_visa_previous_file_upload"
              )
            ) {
              fileNum = $(
                "input[name=additional_australia_visa_previous_file_upload"
              ).get(0).files.length;
              valueStr =
                fileNum > 1 ? fileNum + " files" : studyFormArray[stf].value;
            }
  ...
  $(".study-info-primary-summary-container").html(
    '<table class="application-summary-tbl">' + pgPrimaryLabelHtml + "</table>"
  );
  ...
  if (programAlternateLabelArray.length > 0) {
    for (let pa = 0; pa < programAlternateLabelArray.length; pa++) {
      pgAlternateLabelHtml +=
        '<tr><td><span class="bold">' +
        programAlternateLabelArray[pa].label +
        '</span></td><td><span class="value-input">' +
        programAlternateLabelArray[pa].value +
        "</span></td></tr>";
    }
    $(".study-info-alternate-summary-container").html(
      '<div class="study-box-container"><h4>+Greystone College</h4><table class="application-summary-tbl">' +
        pgAlternateLabelHtml +
        "</table></div>"
    );
  }
  ...
  if (accommodationLabelArray.length > 0) {
    for (let acc = 0; acc < accommodationLabelArray.length; acc++) {
      valueStr =
        accommodationLabelArray[acc].value == 1
          ? "Yes"
          : accommodationLabelArray[acc].value == 0
          ? "No"
          : accommodationLabelArray[acc].value;
      accLabelHtml +=
        '<tr><td><span class="bold">' +
        accommodationLabelArray[acc].label +
        '</span></td><td><span class="value-input">' +
        valueStr +
        "</span></td></tr>";
    }
    accLabelHtml =
      '<table class="application-summary-tbl">' + accLabelHtml + "</table>";

    if (homestayLabelArray.length > 0) {
      for (let hs = 0; hs < homestayLabelArray.length; hs++) {
        hsLabelHtml +=
          '<tr><td><span class="bold">' +
          homestayLabelArray[hs].label +
          '</span></td><td><span class="value-input">' +
          homestayLabelArray[hs].value +
          "</span></td></tr>";
      }
      hsLabelHtml =
        '<h5>Additional Details</h5><table class="application-summary-tbl">' +
        hsLabelHtml +
        "</table>";
    }
    ...
    if (countrySelected == "Online") {
      $(".accommodation-info-summary-container")
        .addClass("study-hide")
  ...
  for (let ad = 0; ad < additionalFormArray.length; ad++) {
    if (additionalFormArray[ad].name.includes("additional_family")) {
      ...
      let arrTarget = schoolVal == 'Greystone Institute' ? flattenFields(additionalAdditionalV2Array) : additionalFormArray[ad].name.includes("file") ? fileUploadArray : flattenFields(additionalInfoArray);
  ...
  for (let add = 0; add < additionalLabelArray.length; add++) {
    valueStr =
      additionalLabelArray[add].value == 1
        ? "Yes"
        : additionalLabelArray[add].value == 0
        ? "No"
  ...
  if (fileUploadLabelArray.length > 0) {
    for (let f = 0; f < fileUploadLabelArray.length; f++) {
      fileLabelHtml +=
        '<tr><td><span class="bold">' +
        fileUploadLabelArray[f].label +
        '</span></td><td><span class="value-input">' +
        fileUploadLabelArray[f].value +
        "</span></td></tr>";
    }
    fileLabelHtml =
      '<h5>File Uploads</h5><table class="application-summary-tbl">' +
      fileLabelHtml +
      "</table>";
  }
  ...
  if (programAdditionalArray.length > 0) {
    ...
    for (let ap = 0; ap < values.length; ap++) {
      ...
      programInt = ap + 2;
      $(".study-info-program-additional-summary-container").append(
        headerLabel +
          '<table class="application-summary-tbl" style="width:100%;margin-bottom:30px;"><th>+Program #' +
          programInt +
          "</th>" +
          pgAdditionalLabelHtml +
          "</table>"
  ...
  (api_url = encodeURI(api_url)),
    $.ajax({
      ...
      success: function (data) {
        let dataObject = data.results,
          ...
          inputTxt =
            countrySelected == "Canada"
              ...
              : countrySelected == "USA"
              ? releaseFormStatementHtml + proofOfFundsHtml
        ...
        policyHtml =
          '<div class="footnote">' + contractTxt + inputTxt + "</div>";
...
const printPersonForm = function (
  formArray,
  enabled,
  categoryName,
  className,
  ...
  for (let stForm in result) {
    ...
    helpTxt = result[stForm].helpText
      ? '<sup><i class="fas fa-info-circle" data-container="body" data-trigger="hover" data-toggle="popover" data-placement="right" data-content="' +
        ...
        '"></i></sup>'
      : "";
    requiredValue = result[stForm].requiredDependence
      ? result[stForm].requiredDependence.includes(schoolVal)
        ? "required"
        : ""
      : result[stForm].required == true
      ? "required"
      : "";
    inputLabel =
    ...
    applicationStatus = !result[stForm].applicationRestriction
      ? true
      : ar.indexOf(applicationName) != -1
      ? false
      : true;
        ...
        if (!enabled) {
          if (result[stForm].category == categoryName) {
            switch (result[stForm].inputType) {
                  ...
                  if (result[stForm].source) {
                    if (result[stForm].source == "local-storage") {
                      optionArray = JSON.parse(
                        localStorage.getItem(result[stForm].obj)
                      );
              ...
              case "file":
                inputForm =
                  "<input type='file' class='form-control additional-file-input' id='" +
                  labelId +
                  "' name='" +
                  inputName +
                  "' " +
              ...
              case "date-time-expiry":
                inputForm =
                  "<input type='text' value='" +
                  value +
                  "' class='form-control date-picker-expiry' id='" +
                  categoryName +
                  "-" +
                  inputName +
                  "-datePicker-" +
              ...
              case "checkbox-option":
                ...
                if (result[stForm].obj) {
                  for (let optionList in result[stForm].obj) {
                    checkInputHtml +=
                      "<div class='checkbox-line'><input type='checkbox' value='" +
                      result[stForm].obj[optionList].optionValue +
                      "' class='form-control' id='" +
                      labelId +
                      dInt +
                      "' name='" +
                      ...
                      dInt +
                      ">" +
                      result[stForm].obj[optionList].optionLabel +
                      helpTxt +
                      "</label></div>";
                  }
                }
              ...
              case "radio-option":
                if (result[stForm].obj) {
                  for (let optionList in result[stForm].obj) {
                    ...
                    target = result[stForm].obj[optionList].optionTarget ? result[stForm].obj[optionList].optionTarget : false;
                    ...
                    otherClass =
                      ...
                      result[stForm].obj[optionList].optionType ==
                        "option-multi"
                        ? "other-input"
                        : result[stForm].obj[optionList].optionType == "option-other-multi-input" ? "option-other-multi-input" : result[stForm].obj[optionList].optionType == "option-other-multi-input-hide" ? "option-other-multi-input-hide" : "independent-input";
                    ...
                    if (
                      result[stForm].obj[optionList].optionType == "option-add"
                    ) {
                      optionHtml +=
                    ...
                    if (
                      result[stForm].obj[optionList].optionType ==
                      "option-other-multi-input"
                    ){
                      optionCondition = result[stForm].obj[optionList].optionCondition ? result[stForm].obj[optionList].optionCondition : false;

                      formArray = optionCondition ? result[stForm].obj[optionList].conditionArray : result[stForm].obj[optionList].obj;
                      target = result[stForm].obj[optionList].optionTarget ? result[stForm].obj[optionList].optionTarget : false;

                      optionChildHtml += printOptionForm(formArray,result[stForm].obj[optionList].dependentClass,categoryName, target,optionCondition,result[stForm].obj[optionList].placeholder,optionList);
                    }
                  }
                }
                inputForm = optionHtml + optionChildHtml;
                break;
            }
...
const printOptionForm = function (arr,dependentClass,categoryName,target, optionCondition, placeholder, iterate) {
  let selectedProgramOptionName = $("input[name=program_option_primary-input]:checked").val(),
      optionHtml = "",
      inputItem = "",
      targetAttr = target ? "data-target='" + target + "'" : "",
      optionObj = optionCondition ? arr.filter(item => selectedProgramOptionName.includes(item.conditionProgramType)) : [];

      formArray = optionCondition ? optionObj[0].obj : arr;

  for (let i = 0; i < formArray.length; i++) {
    digitalAuthority = formArray[i].digitalAuthority ? "data-authority='true' data-target='" + formArray[i].optionTarget + "'" : "";
    
    switch (formArray[i].inputType) {
      case "text":
        inputItem += "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><input type='text' data-category='" + categoryName + "' class='form-control' id='" +
          formArray[i].inputName + "-" + iterate + "' name='" + formArray[i].inputName + "' value='' " + digitalAuthority + " required disabled></div>";
        break;
      case "text-date":
        inputItem += "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><input type='text' data-category='" + categoryName + "' onfocus='(this.type=\"date\")' onblur='(this.type=\"text\")' class='form-control' id='" +
          formArray[i].inputName + "-" + iterate + "' name='" + formArray[i].inputName + "' value='' placeholder='YYYY-MM-DD' required disabled></div>";
        break;
      case "dropdown":
        optionList = "";
        dropArray = formArray[i].obj;
        ...
        inputItem +=
          "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><select class='form-control' data-category='" + categoryName + "' id='" +
          formArray[i].inputName +
          "-" + iterate + "' name='" +
          formArray[i].inputName +
          "' " +
          " required disabled><option disabled selected value=''>Choose " +
          formArray[i].inputLabel +
          "</option>" +
          optionList +
          "</select></div>";
        break;
      case "text-phone":
          inputItem +=
            "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><input type='tel' class='form-control' data-category='" + categoryName + "' id='" +
            formArray[i].inputName +
            "-" + iterate + "' name='" +
            formArray[i].inputName +
            "' " +
            "' placeholder='example: +1 123 123 12345' value='' required disabled></div>";
          break;
      case "text-email":
            inputItem +=
              "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><input type='email' class='form-control' data-category='" + categoryName + "' id='" +
              formArray[i].inputName +
              "-" + iterate + "' name='" +
              formArray[i].inputName +
              "' " +
              "' placeholder='example: john.doe@example.com' value='' required disabled></div>";
            break;
      case "file":
            inputItem +=
              "<div class='form-group'><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label><input type='file' class='form-control additional-file-input' data-category='" + categoryName + "' id='" +
              formArray[i].inputName +
              "-" + iterate + "' name='" +
              formArray[i].inputName +
              "' data-bind='" +
                formArray[i].objInputName +
                "' accept='jpeg,jpg,pdf,png' " +
              "' required disabled><input type='hidden' name='" + formArray[i].inputName + "' value='' readonly disabled></div>";
            break;
      case "checkbox":
          digitalAuthorityClassName = formArray[i].digitalAuthority ? "jr-confirmation" : "";
          inputItem +=
            "<div class='form-group'><div class='checkbox-line'><input class='" + digitalAuthorityClassName + "' type='checkbox' " + digitalAuthority + " data-category='" + categoryName + "' id='" +
            formArray[i].inputName +
            "-" + iterate + "' name='" +
            formArray[i].inputName +
            "' value='' required disabled><label for='" + formArray[i].inputName + "-" + iterate + "'>" + formArray[i].inputLabel + "</label></div></div>";
    }
  }
  optionHtml = '<div class="multi-option-form dependent-target study-box-container study-hide ' + dependentClass + '" style="padding-left:50px;" ' + targetAttr + '><h5>' + placeholder + '</h5>' + inputItem + '</div>';
  return optionHtml;
}
const printStudentForm = function () {
  ...
  if (applicationStatus == "Agent") {
    ...
    applicationStatus == "Walk-in Student Advisor" ||
    applicationStatus == "Direct Sales"
  ) {
    //printAgentForm();
...
const printAdditionalInputForm = function(arr,categoryName){
  let categoryHtml = "",
      optionHtml = "",
      titleHtml = "",
      inputItem = "",
  ...
  for (const categoryTitle in groupedArray) {
    // Check if the property belongs to the object itself
    if (Object.prototype.hasOwnProperty.call(groupedArray, categoryTitle)) {
...
const getSchoolCountryList = function () {
  let dropElement =
    ...
    schoolParentQuery =
      schoolVal == "ELS" ? "&school_available__in=ELS" : schoolVal == "Greystone Institute" ? "&school_available__in=Greystone Institute" : "&school_available__in=Language School,Greystone College",
    queryParam =
      ...
      .done(function (data) {
        ...
        if (dataObject.length > 0) {
          for (let i = 0; i < dataObject.length; i++) {
            ...
            listHtml =
              "<li class='dropdown-item' data-payment='" +
              paymentEnable +
              "' data-currency='" +
              currencyValue +
              "' data-value='" +
          ...
          $(
            ...
            $("input[name=program_country]")
              .val(selectedValue)
              .trigger("change");
            $("input[name=program_country_currency]")
              .val(selectedCurrencyValue)
              .trigger("change");
            $("input[name=program_country_payment_enable]")
              .val(selectedPaymentEnable)
              .trigger("change");
...
const resetJuniorWaiverInfo = function () {
  $("#additional-jr-waiver-container").empty();
};
...
const resetProgramFormField = function (
  elementStatus,
  formElement,
  emptyElement,
  inputName
  ...
  if (elementStatus == "primary-input") {
    $("#study-program-" + comboVal + " .study-program-alternate")
      .removeClass("study-show")
      .addClass("study-hide");
  }
  if (formElement) {
    $(formElement, "#study-program-" + comboVal)
      .removeClass("study-show")
      .addClass("study-hide");
...
const resetAccommodationDetails = function () {
  ...
  $("input,select,textarea", ".accommodation-airport-transfer-details").attr(
    "disabled",
    true
  );
  $(".accommodation-airport-transfer-details")
    .removeClass("study-show")
    .addClass("study-hide");
};
...
const printWeeksOption = function (selectedProgramArray, element, inputName) {
  let selectedSchool = $("input[name=program_school]:checked").val(),
    ...
    durationString = selectedSchool == "Greystone Institute" ? " years" : " weeks",
    labelHtml = "<label>" + labelString + "</label>",
    optionHtml = "";
  if (dataObject.length > 0) {
    for (let i = 0; i < dataObject.length; i++) {
      optionHtml +=
        ...
        '">' +
        dataObject[i].program_length + durationString +
        "</label></div>";
    }
    $(
      "#study-program-" +
...
const printSkillPlusFileForm = function () {
  ...
  className =
    ".study-program-selection .input-container.skills-plus-initial .skill-plus-class-file-container";
...
const printSkillsPlusProgram = function () {
  let tableId = programTableLS,
    ...
    selectedLocation = $(
      "select[name=program_name_primary-input] option:selected"
    ).attr("data-location"),
    selectedSchool = "81771084435",
    queryLocation = selectedLocation
      ? "&country__in=" + selectedLocation
      : "&campus__in=" + selectedCampus,
    queryParam =
      queryLocation +
      "&schools__in=" +
      selectedSchool +
      "&category__eq=Skills Plus&enable=1",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam;
  (api_url = encodeURI(api_url)),
  ...
  if ($(selectEl).is(":empty")) {
    $.ajax({
      ...
      success: function (data) {
        ...
        if (dataObject.length > 0) {
          optionHtml =
            '<option value="" selected disabled>Choose Skills Plus Class</option>';
          ...
          optionHtml =
            '<option value="" selected disabled>No Skills Plus Available at this time.</option>';
        }

        $(selectEl).html(optionHtml).attr("disabled", false);
        $(".skill-plus-class-drop-container")
          .removeClass("study-hide")
          .addClass("study-show");
      },
    });
    ...
    $(".skill-plus-class-drop-container")
      ...
      .addClass("study-show");
    if (
      !$("select[name=program-skills-plus-start-date_primary-input]").is(
        ":empty"
      )
    ) {
      $(".skill-plus-class-date-container")
        .removeClass("study-hide")
        .addClass("study-show");
...
const printSkillsPlusStartDate = function () {
  let tableId = startDateTableLS,
    ...
    queryLocation = selectedLocation
      ? "&location__in=" + selectedLocation
      : "&campus__in=" + selectedCampus,
    program = $(
      "select[name=program-skills-plus-class_primary-input] option:selected"
    ).attr("data-id"),
    startDateSelected = $(
    ...
    durationSelectedVal = parseInt(
      durationSelected.replace("weeks", "").replace("week", "")
    ),
    primaryEndDate = addWeeks(
      new Date(parseInt(startDateSelected)),
      durationSelectedVal
    ),
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (dataObject.length > 0) {
        optionHtml =
          '<option value="" selected disabled>Choose Skills Plus Class Start Date</option>';
        ...
        for (let i = 0; i < dataObject.length; i++) {
          ...
          dayVal =
            dayVal.toString().length < 2 ? "0" + dayVal.toString() : dayVal;
          dateInt = yearVal + "-" + monthInt + "-" + dayVal;
          (id = dataObject[i].id), (weekAvailableArray = []);

          if (dataObject[i].values.fixed_week_duration) {
            for (
              let x = 0;
              x < dataObject[i].values.fixed_week_duration.length;
              x++
            ) {
        ...
        optionHtml =
          '<option value="" selected disabled>No dates available.</option>';
      }
...
const printSkillsPlusDuration = function () {
  availableWeeks = $(
    "select[name=program-skills-plus-start-date_primary-input] option:selected"
  ).attr("data-week-available");
  availableWeeksArray = availableWeeks.split(",");
  availableWeeksArray.sort(function (a, b) {
    return a - b;
  });
  primaryDurationSelected = $(
    "select[name=program_option_duration_primary-input] option:selected"
  ).val();
  primaryDuration = parseInt(
    primaryDurationSelected.replace("weeks", "").replace("week", "")
  );
  ...
  $("select[name=program-skills-plus-duration_primary-input]")
    .html(optionHtml)
    .attr("disabled", false);
  $(".skill-plus-class-duration-container")
    .removeClass("study-hide")
    .addClass("study-show");
};
...
const resetSkillsPlusStartDate = function () {
  skillsEnable =
    $("input[name=is-skills-plus-apply]:checked").val() == "Yes" ? true : false;
  programEnable = $(
    "select[name=program-skills-plus-class_primary-input]"
  ).val()
    ? true
    : false;
...
const printMicroScheduleOption = function (
  selected,
  element,
  inputName,
  programType
  ...
  if (dataObject.length > 0) {
    for (let i = 0; i < dataObject.length; i++) {
      ...
      optionHtml +=
        ...
        '" type="radio" name="program_schedule_' +
        element +
        comboName +
        '" data-weeks="' +
    ...
    if (dataObject.length == 1) {
      $("input[name=program_weeks_" + element + comboName)
        .val(dataObject[0].program_length)
        .attr("disabled", false);
      inputName = $(
        "input[name*=program_schedule_" + element + comboName + "]:checked"
      ).attr("name");
...
const printScheduleOption = function (
  selectedProgramArray,
  element,
  inputName,
  programType
) {
...
const printProgramOption = function (
  selectedProgram,
  element,
  minAge,
  maxAge,
  ...
  $.get(api_url).done(function (data) {
    ...
    if (dataObject.length > 0) {
      for (let i = 0; i < dataObject.length; i++) {
        ...
        sessionName = dataObject[i].values.day_schedule
          ? dataObject[i].values.day_schedule.name
          : "";
        breakArray.push({
...
const printStartDates = function (
  selectedProgram,
  element,
  minAge,
  maxAge,
  ...
  let comboVal = returnComboVal(inputName),
    ...
    selectedSchedule = $(
      "input[name=program_option_primary-input]:checked"
    ).val(),
    selectedScheduleArray = selectedSchedule ? selectedSchedule.split("-") : "",
    selectedScheduleItem =
      selectedScheduleArray.length > 1 ? selectedScheduleArray[0] : "",
    selectedScheduleName =
      selectedScheduleArray.length > 1 ? selectedScheduleArray[1] : "",
    dateBirth = new Date($("input[name=student_dob]").val()),
    dateBirthTime = dateBirth.getTime(),
    minorAge = 18,
    ...
    availableStartDateMessage =
      schoolVal == "ELS"
        ? " Available start dates depend on age of student upon the start of program."
        : null,
    sessionStartDateMessage =
      schoolVal == "ELS"
        ? "For the best experience we encourage students (especially academically-bound students who plan to progress into an ELS partner school) to start on session start dates and not to start on week four of the session. However, start dates are available every Monday.<br/><br/><sup>*</sup>Session Start Date"
        : "<sup>*</sup>Session Start Date",
    typeQuery = programType == "Micro-Credential" ? "course" : "program",
    programScheduleSelected =
      selectedParentSchool == "Greystone College"
        ? $("input[name=" + inputName + "]:checked").val()
        : "",
    tableId =
      selectedParentSchool == "Language School"
        ? startDateTableLS
        : selectedParentSchool == "Greystone College"
        ? programType == "Micro-Credential"
          ? startDateTableCoursesGC
          : startDateTableGC
        : selectedParentSchool == "ELS"
        ? startDateTableELS
        : selectedParentSchool == "Greystone Institute"
        ? startDateTableGI
        : startDateTableLS,
  ...
  if (element == "alternate-input") {
    (programScheduleSelected = $(
      "input[name=program_schedule_alternate-input]:checked"
    ).val()),
      (tableId =
        programType == "Micro-Credential"
          ? startDateTableCoursesGC
  ...
  selectedCurrentSchool =
    element == "alternate-input"
      ? $("input[name=program_school_alternate-input]").val()
      : selectedParentSchool;

  if (comboVal > 1) {
    previousDateSelected = $("input[name=" + inputName + "]:checked")
      .parents()
      .eq(5)
      .prev()
      .find('select[name*="program_startdate_"] option:selected')
      .attr("data-value");
    previousDurationSelected =
      selectedCurrentSchool == "Greystone College"
        ? parseInt(
            $("input[name=" + inputName + "]:checked")
              .parents()
              .eq(5)
              .prev()
              .find('input[name*="program_weeks_' + element + '"]')
              .val()
          )
        : parseInt(
            $("input[name=" + inputName + "]:checked")
              ...
              .prev()
              .find(
                'select[name*="program_option_duration_' +
                  element +
                  '"] option:selected'
              )
              .val()
    ...
    previousDateSelected =
      element == "alternate-input"
        ? $(
            "select[name=program_startdate_primary-input" +
              comboName +
              "] option:selected"
          ).attr("data-value")
        : $(
            "select[name=program_startdate_" +
              element +
              comboName +
              "] option:selected"
          ).attr("data-value");
    previousDurationSelected =
      element == "alternate-input"
        ? parseInt(
            $(
              "select[name=program_option_duration_primary-input" +
                comboName +
                "] option:selected"
            )
              .val()
        ...
        : selectedCurrentSchool == "Greystone College"
        ? $("input[name=program_weeks_" + element + comboName + "]").val()
        : parseInt(
            $(
              "select[name=program_option_duration_" +
                element +
  ...
  epochPrimaryStartDate =
    comboVal > 1 || element == "alternate-input"
      ? parseInt(primaryStartDate) + 1000 * 60 * 60 * 24 * primaryDuration
      : "";
  ...
  if (comboVal > 1 || element == "alternate-input") {
    ...
    epochPrimaryStartDate =
      epochDay == 1
        ? epochPrimaryStartDate - 86400 * 1000
        : epochPrimaryStartDate;
  ...
  if (typeof tableId !== "undefined") {
    dateQuery =
      epochPrimaryStartDate != ""
        ? "&start_dates__gte=" + epochPrimaryStartDate
        : "";
    sessionSchedule =
      programType == "Micro-Credential"
        ? "&session_schedule__eq=" + programScheduleSelected
        : "";
    programScheduleOption =
      campusName === "Dublin" && selectedScheduleArray.length > 1
        ? "&item__in=" +
          selectedScheduleItem +
          "&schedule_option__in=" +
          selectedScheduleName
      ...
      .done(function (data) {
        ...
        if (dataObject.length > 0) {
          ...
          for (let i = 0; i < dataObject.length; i++) {
            ...
            epochVal = dataObject[i].values.start_dates;
            isSessionDate = dataObject[i].values.session_start_date
              ? "<sup>*</sup>"
              : "";
            ...
            if (currentDate.getTime() <= epochVal) {
              ...
              if (dateBirthTime <= minDob && dateBirthTime >= maxDob) {
                ...
                optionHtml =
                  ...
                  dateInt +
                  "' data-value='" +
                  dataObject[i].values.start_dates +
                  "' data-week-available='" +
                  weekAvailableArray.toString() +
                  "' " +
                  selectedOption +
          ...
          ageQualifyMessage =
            "<option disabled selected value=''>No Start Dates Available. Possible reason - There is an age requirement for this selected program or campus upon the start date of the program.</option>";
          ...
          if (enableOption) {
            ...
            if (availableStartDateMessage) {
              $(
                "#study-program-" +
                  ...
                  " ." +
                  element +
                  " .study-program-start-date-dropdown label .age-message"
              ).remove();
              $(
                "#study-program-" +
                  comboVal +
                  " ." +
                  element +
                  " .study-program-start-date-dropdown label"
              ).append(
                '<p class="age-message">' + availableStartDateMessage + "</p>"
              );
            }
            ...
            $(
              "#study-program-" +
                ...
                " ." +
                element +
                " .study-program-start-date-dropdown label p.session-message"
            ).remove();
            if (hasSessionDate.includes(1)) {
              $(
                ...
                '<p class="session-message">' + sessionStartDateMessage + "</p>"
              );
            }
            if (campusName == "New York") {
              $(
                ...
                '<p class="session-message">' + startDateMessage + "</p>"
              );
            }
          } else {
            $(
  ...
  if (selectedCurrentSchool == "Greystone College") {
    printAddProgramLink(
      $("select[name=program_startdate_" + element + comboName + "]").parent()
    );
  } else {
    removeProgramLink();
  }
...
const printProgramDuration = function (element, inputName) {
  let comboVal = returnComboVal(inputName),
    ...
    durationNote =
      countrySelected == "Ireland" && programSelected !== "Study and Work"
        ? '<div id="duration-note"><p><sup>*</sup>Non EU/EEA students on a Tourist Visa / Visa Waiver may study a maximum of 12 weeks</p></div>'
        : "";
  ...
  if (element == "primary-input") {
    ...
    if (minVal == undefined && maxVal == undefined) {
      lengthVal =
        durationArray.length > 0 ? durationArray.length : maxLimit + 1;
      ...
      coursesHtml =
        selectedCountry != "Australia"
          ? selectedCampus != "52464878081"
            ? '<div class="form-check"><input class="form-check-input ' +
              element +
              '" value="2" type="radio" name="program_is_coop_' +
              element +
              '" id="no-coop-courses" required><label class="form-check-label" for="no-coop-courses">Micro-Credentials</label></div>'
            : ""
          : "";
...
const printProgramDrop = function (
  selectedCampus,
  selectedVal,
  element,
  inputName
  ...
  let selectedSchool = $("input[name=program_school]:checked").attr("id"),
    ...
    queryString = document.location.search,
    dict = parseQueryStringToDictionary(queryString),
    affiliateQuery = dict.affiliate
      ? "&affiliate_partners__in=" + dict.affiliate
      : "";
  ...
  if (selectedParentSchool == "Language School") {
    ...
    (accommObjNum = "enable_accommodations"),
    (minAgeObjNum = "age_minimum"),
    (programAreaObjNum = "program_areas"),
    (coopBool =
        selectedVal == 1
          ? "&is_co_op__eq=1"
          : selectedVal == 0
  ...
  if (typeof campusProp !== "undefined") {
    ...
    $.get(api_url).done(function (data) {
      ...
      if (dataObject.length > 0) {
        if (selectedParentSchool == "Greystone College" || selectedParentSchool == "Greystone Institute") {
          laSalleEligibleArr = [];

          for (let i = 0; i < dataObject.length; i++) {
            minAge = dataObject[i].values[minAgeObjNum]
              ? dataObject[i].values[minAgeObjNum][0].name
              : "";
            parentProgramId = dataObject[i].values.parent_program_code
              ? dataObject[i].values["parent_program_code"][0].id
              : "";
            programSchedule = dataObject[i].values.schedule
              ? dataObject[i].values.schedule
              : "";
            programLength = dataObject[i].values.program_length_weeks
              ? dataObject[i].values.program_length_weeks
              : "";
            isCoop = dataObject[i].values.is_co_op
              ? dataObject[i].values.is_co_op
              : 0;
            ...
            if (dataObject[i].values.lasalle_eligible_campus) {
              if (dataObject[i].values.lasalle_eligible_campus.length > 0) {
                for (
                  let x = 0;
                  x < dataObject[i].values.lasalle_eligible_campus.length;
                  x++
                ) {
            ...
            if (dataObject[i].values[programAreaObjNum]) {
              if (dataObject[i].values[programAreaObjNum].length > 0) {
                for (
                  let x = 0;
                  x < dataObject[i].values[programAreaObjNum].length;
                  x++
                ) {
                  pgrmAreaArr.push(
          ...
          for (let i = 0; i < values.length; i++) {
            for (let x = 0; x < values[i].length; x++) {
              programId =
                programType == "Micro-Credential"
                  ? values[i][x].program_id
                  : values[i][x].parent_program_id;
              if (x == 0) {
          ...
          for (let i = 0; i < dataObject.length; i++) {
            campusTypeArr = [];
            minAge =
              dataObject[i].values[minAgeObjNum] !== undefined
                ? dataObject[i].values[minAgeObjNum][0].name
                : "";
            maxAge =
              dataObject[i].values[maxAgeObjNum] !== undefined
                ? dataObject[i].values[maxAgeObjNum][0].name
                : "";
            seasonVal =
              selectedParentSchool == "Language School"
                ? dataObject[i].values[seasonNum] !== undefined
                  ? dataObject[i].values[seasonNum].name
                  : ""
                : "";
            campusTypeVal =
              selectedParentSchool == "Language School"
                ? dataObject[i].values[campusType] !== undefined
            ...
            if (dataObject[i].values["country"]) {
              if (dataObject[i].values["country"].length > 0) {
                for (
                  let x = 0;
                  x < dataObject[i].values["country"].length;
                  x++
                ) {
                  if (
                    dataObject[i].values["country"][x].name != selectedCountry
                  ) {
            ...
            if (dataObject[i].values[campusType] !== undefined) {
              for (
                let x = 0;
                x < dataObject[i].values[campusType].length;
                x++
              ) {
                campusTypeArr.push(dataObject[i].values[campusType][x].name);
              }
            }
      ...
      if (group_by == "program_area") {
        printDropDownArray.sort(
          (a, b) =>
            a[group_by].localeCompare(b[group_by]) ||
      ...
      if (printDropDownArray.length > 0) {
        optionHtml =
          "<option disabled selected value=''>Choose " + typeText + "</option>";
        ...
        for (let i = 0; i < printDropDownArray.length; i++) {
          ...
          category = printDropDownArray[i].category
            ? printDropDownArray[i].category.name
            : "";
          programArea =
            selectedParentSchool == "Greystone College"
              ? programType == "Micro-Credential"
                ? printDropDownArray[i].program_area
                  ? "[" + printDropDownArray[i].program_area + "] - "
                  : ""
                : ""
              : "";
          programLocation = printDropDownArray[i].program_location
            ? printDropDownArray[i].program_location
            : "";
          ...
          optionHtml =
            ...
            "' data-season='" +
            printDropDownArray[i].season +
            "' data-program-type='" +
            printDropDownArray[i].program_type +
            "' data-category='" +
            category +
            "' data-prola='" +
            isProla +
        ...
        $(
          "#study-program-" +
            ...
            " ." +
            element +
            " .study-program-dropdown select"
        )
          .html(optionHtml)
          .attr("name", "program_name_" + element + comboName)
          .attr("id", "program-name-" + element + comboName)
          .addClass(element);
...
const printCampus = function (selected) {
  ...
  $.get(api_url).done(function (data) {
    let dataObject = data.results;
    dataObject.sort((a, b) =>
      a.values.state_province > b.values.state_province ? 1 : -1
    );
    campusArray = [];
    ...
    if (dataObject.length > 0) {
      for (let i = 0; i < dataObject.length; i++) {
        ...
        boxName = dataObject[i].values.thumnail_images
          ? ""
          : initials.join("").toUpperCase();
        thumbNail = dataObject[i].values.thumnail_images
          ? "style=background-image:url('" +
            dataObject[i].values.thumnail_images.url +
            "') no-repeat;"
          : "";
        minAge =
          schoolVal == "ELS" || schoolVal == "Greystone Institute"
            ? dataObject[i].values.min_age
              ? dataObject[i].values.min_age
              : ""
            : "";
        campusArray.push({
      ...
      for (let x in groupArray) {
        ...
        for (let i = 0; i < groupArray[x].length; i++) {
          ...
          listHtml +=
            ...
            groupArray[x][i].id +
            '" class="box-selection"><div class="box-color" data-min-age="' +
            groupArray[x][i].minAge +
            '" data-value="' +
            groupArray[x][i].id +
            '" ' +
...
const printSchoolSelection = function () {
  let countrySelected = $(
    ...
    countryQuery =
      schoolVal == "ELS" ? "&parent_school__in=ELS" : schoolVal == "Greystone Institute" ? "&parent_school__in=Greystone Institute" : "&parent_school__in=Language School,Greystone College",
    queryParam =
      countryQuery +
      "&location_country__in=" +
      countrySelected +
      "&enable__eq=true",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam,
    listHtml = "";
    ...
    if (dataObject.length > 0) {
      for (let i = 0; i < dataObject.length; i++) {
        ...
        if (dataObject[i].values.available_countries) {
          ...
          for (
            let x = 0;
            x < dataObject[i].values.available_countries.length;
            x++
          ) {
            nationalitiesArr.push(
              dataObject[i].values["available_countries"][x].name
            );
          }
        ...
        if (printSchoolVar) {
          initials =
            dataObject[i].values["parent_school"][0].name.match(/\b(\w)/g);
          boxName = dataObject[i].values.thumbnail_images
            ? ""
            : initials.join("").toUpperCase();
          thumbNail = dataObject[i].values.thumbnail_images
            ? "style=background-image:url('" +
              dataObject[i].values.thumbnail_images.url +
              "') no-repeat;"
            : "";
          ...
          listHtml +=
            ...
            dataObject[i].values.is_pairable +
            '" value="' +
            dataObject[i].values.name +
            '" data-school="' +
            dataObject[i].values["parent_school"][0].name +
            '" id="' +
            dataObject[i].id +
            '" required><span>' +
            dataObject[i].values.name +
...
const printAdditionalInfo = function () {
  let countrySelected = $("input[name=program_country]").val(),
    ...
    programCategorySelected = $(
      "select[name*=program_name_] option:selected"
    ).attr("data-category"),
    durationSelected = $(
      "select[name*=program_option_duration_] option:selected"
    ).val(),
    durationSelectedVal = parseInt(
  ...
  if (schoolVal == "Greystone Institute") {
    ...
    if (countrySelected == "Australia") {
      ...
      jrFamilyHtml =
        programCategorySelected == "Family Camps" ||
        programSelected.includes("Winter Experience")
          ? jrFamilyMemberButtonHtml
          : "";
      shirtHtml =
        programCategorySelected == "Junior Camps" ||
        programCategorySelected == "Family Camps"
      ...
      inputHtml =
        selectedParentSchool == "Greystone College" ||
        isProla ||
        $("input[name=is_alternate_apply][value=1]").is(":checked")
          ? prolaHtml
          : "";
      fileInputHtml =
        programSelected != "French Communication Evening"
          ? insidePermitFileHtml + insideFlightFileHtml
          : "";
      additionalHtml =
        shirtHtml +
        jrFamilyHtml +
      ...
      additionalHtml =
        inputHtml +
        additionalCurrencyPaymentOptionHtml +
        additionalOnlineNotesHtml;
    }
  }
  ...
  if ($(".additional-notes-container .additional-notes-input").is(":empty")) {
    ...
    if(schoolVal == "Greystone Institute"){
      topPos = $(".additional-notes-container .additional-notes-input").offset().top - 300;
      $('html, body').animate({
          scrollTop: topPos
      }, 300);
    }
...
const hideShowIForm = function (programName, duration) {
  if (programName == "American Explorer") {
    $("input[type=radio][name=additional_visa_form_us]")
      .attr("disabled", true)
      .prop("checked", false);
    $("input[type=radio][name=additional_visa_form_us][value=No]").prop(
      "checked",
      true
    );
    $("input[type=hidden][name=additional_visa_form_us]").attr(
      "disabled",
      false
    );
    $(".additional-notes-input .visa-form-input")
      .addClass("study-hide")
      .removeClass("study-show");
    ...
    if (duration < 4) {
      ...
      $(".additional-notes-input .visa-form-input")
        ...
        .removeClass("study-show");
      $(
        ".additional-notes-input .visa-form-input .dependents-container"
      ).empty();
    } else {
      $("input[type=hidden][name=additional_visa_form_us]").attr(
        "disabled",
      ...
      $("input[type=radio][name=additional_visa_form_us]").attr(
        "disabled",
        false
      );
    }
...
const uploadFile = function (dataStep) {
  ...
  if (dataStep == "additional-select") {
    ...
    hsFormId = accommodationUploadFormId;
    targetId = "accommodation-hs-file-form";
    submitVar = true;
  }
...
const submitForm = function (dataStep) {
  ...
  switch (dataStep) {
    ...
    case "accommodation-select":
      ...
      uploadFile("accommodation-select");
  ...
  if (typeof formName !== "undefined") {
          ...
          if (formTempArray[i].name == arr[x].inputName) {
            if (arr[x].objInputName && arr[x].objInputName != "email") {
              if (formName == "form-student") {
                booleanVar =
                  formTempArray[i].name == "student_first_name" ||
                  formTempArray[i].name == "student_middle_name" ||
                  formTempArray[i].name == "student_last_name"
                    ? titleCase(formTempArray[i].value)
                    : formTempArray[i].value;
              } else {
                if (formTempArray[i].name.includes("program_is_coop_")) {
                  booleanVar = formTempArray[i].name.includes(
                    "program_is_coop_"
                  )
                    ? formTempArray[i].value == 1
                      ? "Co-Op"
                      : formTempArray[i].value == 0
                      ? "Academic"
                      : formTempArray[i].value == 2
                      ? "Micro-Credentials"
                      : formTempArray[i].value
                    : formTempArray[i].value == 1
                    ? "Yes"
    ...
    if (additionalProgramTempArray.length > 0) {
      for (let i = 0; i < additionalProgramTempArray.length; i++) {
        booleanVar = additionalProgramTempArray[i].name.includes("program_type")
          ? additionalProgramTempArray[i].value == 1
            ? "Co-Op"
            : additionalProgramTempArray[i].value == 0
            ? "Academic"
            : additionalProgramTempArray[i].value == 2
            ? "Micro-Crendentials"
            : additionalProgramTempArray[i].value
          : additionalProgramTempArray[i].value == 1
          ? "Yes"
          : additionalProgramTempArray[i].value == 0
          ? "No"
    ...
    if (programAdditionalArray.length > 0) {
      ...
      (values = Object.values(groupProgramArray)), (prgmGroup = "");
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        for (let x = 0; x < values[i].length; x++) {
          ...
          addPrgmStr =
            ...
            '"';
          addPrgmStr =
            values[i].length - 1 == x ? addPrgmStr : addPrgmStr + ",";
        }
        prgmGroup =
          values.length - 1 == i
...
const autoHomestaySelect = function (enable) {
  let programSelected = $("select[name*=program_name_] option:selected").val(),
    campusTypeSelected = $("select[name*=program_name_] option:selected").attr(
      "data-campus-type"
    );

  if (enable) {
    $(".accommodation-choice-container")
      .addClass("study-hide")
      .removeClass("study-show");
    $("#yes-accomm-option").click();
    $(".accommodation-type-option-container")
      .addClass("study-hide")
      .removeClass("study-show");
    setTimeout(() => {
    ...
    $(".accommodation-details")
      .addClass("study-hide")
      .removeClass("study-show");
    $("input,select",".accommodation-details").prop("disabled", true);
    $(
      "input[name=accommodation_checkin_datepicker],input[name=accommodation_checkout_datepicker]"
    ).prop("disabled", true);
...
const moveAirportTransfer = function (enable) {
  if (enable) {
    ...
    $(".additional-airport-transfer-details")
      .removeClass("study-hide")
      .addClass("study-show");
    $(
      ".additional-airport-transfer-details input[name=accommodation_airport_transfer_option]"
    ).prop("disabled", false);
  } else {
...
const hideInsurance = function () {
  ...
  $(
    ".insurance-container #insurance-option-container .input-container input[name=insurance_option]"
  )
    .prop("disabled", true)
    .prop("required", false);
  $(
    ".insurance-container #insurance-option-container .input-container input.medical-date-picker"
  ).prop("disabled", true);
};
...
$(document).on(
  "click",
  ".section-application-form .step-container .btn.next",
  function () {
    let dataStepVar = $(this).attr("data-step"),
      ...
      affiliateEnable = dict.affiliate ? true : false,
      slideNext = false;
    validVar = !affiliateEnable
      ? validateForm(dataStepVar, dataForm, validCheck)
        ? true
        : false
      : false;

    if (validVar && !affiliateEnable) {
      if (dataStepVar == "study-program-select") {
        let schoolNameSelected = $("input[name=program_school]:checked").val(),
          countrySelected = $("input[name=program_country]").val(),
          accommodationEnable = parseInt(
            $("select[name=program_name_primary-input] option:selected").attr(
              "data-accommodation-enable"
            )
          ),
...
const programAction = function (tEl, element, pair) {
  ...
  if (selectedParentSchool == "Greystone College" || selectedParentSchool == "Greystone Institute" || pair) {
    let programObject = localStorage.getItem(programObjGC),
      ...
      programSelected =  selectedParentSchool == "Greystone Institute" ? programArray.filter(
        (v) => v.parent_program_id == selected
      ) : programArray.filter(
...
const programOptionAction = function (tEl, element) {
  let selected = $("select.study-program-select option:selected").attr(
    ...
    inputName = $(tEl).attr("name"),
    programType = $("select.study-program-select  option:selected").attr(
      "data-program-type"
    );
...
const resetLaSalleOption = function (element, inputName) {
  ...
  $("#study-program-" + comboVal + " .partner-input." + element)
    .removeClass("study-show")
    .addClass("study-hide");
  $(
    "#study-program-" + comboVal + " .partner-input." + element + " input"
  ).attr("disabled", true);
};
...
const printAddProgramLink = function (element) {
  ...
  if ($(element).find(".add-program-link").length == 0) {
    $(element).append(
      '<a class="add-program-link">' +
        textLink +
        '</a><span class="add-program-warning"></span>'
    );
  }
};
...
$(document).on("click", ".program-multi-box .fa-minus-square", function () {
  let element = $(this).hasClass("alternate-input")
      ? "alternate-input"
      : "primary-input",
    inputName = $(this)
      .parent()
      .prevAll("div[data-program-iterate=1]")
...
$(document).on("click", "a.add-program-link", function () {
  let iterateNum =
    $(".program-multi-box").length == 0
      ? 2
      : $(".program-multi-box").length + 2;
  ...
  if ($(".program-multi-box").length < 5) {
    ...
    $("span.add-program-warning").html(
      "You have reached the maximum number of additional programs (5)."
    );
  }
});
...
const resetProgramComboDate = function (element, inputName) {
  let comboVal = returnComboVal(inputName),
    comboName = returnComboName(inputName),
    parentInt =
      inputName.includes("program_name") ||
      inputName.includes("program_startdate")
        ? 4
        : 5,
    parentStr =
      inputName.includes("program_name") ||
  ...
  if (parentElementMulti.length > 0) {
    ...
    parentElementMulti.each(function (index) {
      if (!$(this).find('select[name*="program_startdate"]').attr("disabled")) {
        if (
          $(this).find('select[name*="program_startdate"] option').length > 0
        ) {
          (programStartDate = $(
            "select[name=program_startdate_" +
              element +
            ...
            (programId =
              selectedCurrentSchool == "Greystone College"
                ? $(this)
                    .find(
                      'input[name*="program_schedule_' + element + '"]:checked'
                    )
                    .attr("id")
                : selectedCurrentSchool == "Language School"
                ? $(this)
                    .find(
                      'input[name*="program_option_' + element + '"]:checked'
                    )
                    .attr("id")
                : $(this)
                    .find(
                      'select[name*="program_name_' +
                        element +
                        '"] option:selected'
                    )
                    .val()),
            (minAge = $(this)
              .find(
                'select[name*="program_name_' + element + '"] option:selected'
              )
              .attr("data-min-age")),
            (maxAge = $(this)
              .find(
            ...
            (inputTarget =
              selectedCurrentSchool == "Greystone College"
                ? $(this)
                    .find('input[name*="program_schedule_' + element + '"]')
                    .attr("name")
                : selectedCurrentSchool == "Language School"
                ? $(this)
                    .find('input[name*="program_option_' + element + '"]')
                    .attr("name")
                : $(this)
                    .find('input[name*="program_name_' + element + '"]')
                    .attr("name"));
          programType = $(this)
            .find(
              'select[name*="program_name_' + element + '"] option:selected'
...
const startDateAction = function (element, thisElement) {
  ...
  if (selectedCountry != "Online" && selectedParentSchool != "ELS") {
    if (element != "alternate-input") {
      printOnlineOption(element);
    }
  }
...
$(document).on(
  "click",
  '.primary-input input[name*="program_is_coop"],.alternate-input input[name*="program_is_coop"]',
  function () {
    let element = $(this).hasClass("alternate-input")
      ? "alternate-input"
      : "primary-input";
...
$(document).on(
  "click",
  '.primary-input input[name*="program_option"]',
  function () {
    let element = $(this).hasClass("alternate-input")
      ? "alternate-input"
      : "primary-input";
...
$(document).on(
  "click",
  '.primary-input input[name*="program_schedule"],.alternate-input input[name*="program_schedule"]',
  function () {
    let element = $(this).hasClass("alternate-input")
      ? "alternate-input"
      : "primary-input";
...
$(document).on(
  "click",
  '.primary-input input[name*="option_lasalle_parthership"],.alternate-input input[name*="option_lasalle_parthership"]',
  function () {
    let element = $(this).hasClass("alternate-input")
      ? "alternate-input"
      : "primary-input";
...
$(document).on(
  "change",
  '.primary-input select[name*="program_option_duration"]',
  function () {
    let element = $(this).hasClass("alternate-input")
        ? "alternate-input"
        : "primary-input",
...
$(document).on(
  "change",
  "select[name=program-skills-plus-start-date_primary-input]",
  function () {
...
$(document).on(
  ...
  function () {
    ...
    if (selectedValue == 1) {
      ...
      $(".study-program-alternate input,.study-program-alternate select").attr(
        "disabled",
        false
      );
    } else {
      ...
      $(".study-program-alternate")
        ...
        .addClass("study-hide");
      $(
        ".program-input.alternate-input,.schedule-input.alternate-input,.date-input.alternate-input"
      )
        .removeClass("study-show")
        .addClass("study-hide");
      $(".study-program-alternate input,.study-program-alternate select").attr(
...
$(document).on(
  "click",
  "input[name=additional_australia_current_study]",
  function () {
...
$(document).on(
  ...
  function () {
    ...
    if ($(this).val() == "Yes") {
      $(el).removeClass("study-hide").addClass("study-show");
      $("input,select", el).attr("required", true).attr("disabled", false);
    } else {
      $(el + "," + el2)
        .removeClass("study-show")
        .addClass("study-hide");
...
$(document).on(
  ...
  function () {
    ...
    if ($("#accordionInstitute #college-institute option").length == 0) {
    ...
    if ($(this).val() == "Yes") {
      $(el + "," + elAccordion)
        .removeClass("study-hide")
        .addClass("study-show");
      $("input,select", el + "," + elAccordion + "," + el2)
        .attr("required", true)
        .attr("disabled", false);
    } else {
      $(el + "," + elAccordion)
...
$(document).on(
  "change",
  "select[name=college-accepted-name],input[name=additional_college_name_input]",
  function () {
    let val = $(this).val();
    ...
    if ($(this).attr("name") === "college-accepted-name") {
      $("input[name=additional_college_name_input]")
        .val("")
        .attr("required", false);
    } else {
      $("select[name=college-accepted-name] option[value=none]").prop(
        "selected",
        true
      );
...
$(document).on(
  "click",
  "#accordionInstitute h4.accordion-header button",
  function () {
...
$(document).on("click", "#add-jr-family-member", function () {
  ...
  if (dValue < 6) {
    $(".jr-family-member-container").append(
      '<div class="' +
        dClass +
        ' d-class"><i class="far fa-minus-square"></i><h4>Family Member #<span class="int-d-val">' +
        dValue +
        "</span></h4>"
    );
    printPersonForm(
      juniorFamilyMembersInputArray,
      false,
      "FamilyMember",
      dClass,
      dValue
...
$(document).on(
  "click",
  ".jr-family-member-container .d-class i.fa-minus-square",
  function () {
    $(this).parent().remove();
    $("form#study-form").attr("data-submit", false);
...
$(document).on(
  "click",
  ".dependents-container .d-class i.fa-minus-square",
  function () {
    $(this).parent().remove();
    $("form#study-form").attr("data-submit", false);
