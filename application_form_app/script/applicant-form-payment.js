...
const printProgramDrop = function (selectedCampus, selectedVal, element, inputName) {
    ...
    let selectedSchool = $('input[name=program_school]:checked').attr('id'),
        ...
        queryString = document.location.search,
        dict = parseQueryStringToDictionary(queryString),
        affiliateQuery = dict.affiliate ? '&affiliate_partners__in=' + dict.affiliate : '';
    ...
    if (typeof campusProp !== 'undefined') {
        let queryParam = '&' + campusProp + '__in=' + selectedCampus + '&schools__in=' + selectedSchool + '&enable=1' + coopBool + '&show_quote_tool=1' + addOnQuery + affiliateQuery,
            api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam;
        api_url = encodeURI(api_url);
        $.get(api_url).done(function (data) {
            let dataObject = data.results;
...
$(document).on("click", ".section-application-form .step-container .btn.next", function () {
    let dataStepVar = $(this).attr('data-step'),
        ...
        affiliateEnable = dict.affiliate ? true : false,
        slideNext = false;
    validVar = !affiliateEnable ? validateForm(dataStepVar, dataForm, validCheck) ? true : false : false;
