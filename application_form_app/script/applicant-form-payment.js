    ...
    if (studyFormArray[stf].name.includes("dependent")) {
      ...
      for (let pa = 0; pa < programInfoArray.length; pa++) {
        ...
        if (splitName[0] == programInfoArray[pa].inputName) {
          ...
          labelValue =
            schoolSelected == "Greystone College"
...
const printOptionForm = function (arr,dependentClass,categoryName,target, optionCondition, placeholder, iterate) {
  ...
  for (let i = 0; i < formArray.length; i++) {
    digitalAuthority = formArray[i].digitalAuthority ? "data-authority='true'" : "";
    ...
    switch (formArray[i].inputType) {
      ...
      case "dropdown":
        ...
        for (let i = 0; i < dropArray.length; i++) {
            optionList +=
              "<option value='" +
              dropArray[i]["optionValue"] +
              "' data-id='" +
              dropArray[i]["optionLabel"] +
              "'>" +
              dropArray[i]["optionLabel"] +
              "</option>";
          }
...
const getSchoolCountryList = function () {
  let dropElement =
    ...
    schoolParentQuery =
      schoolVal == "ELS" ? "&country__eq=USA" : "&country__neq=USA",
    queryParam =
...
const printSchoolSelection = function () {
  let countrySelected = $(
    ...
    countryQuery =
      schoolVal == "ELS" ? "&parent_school__in=ELS" : "&parent_school__ne=ELS",
    queryParam =
      countryQuery +
