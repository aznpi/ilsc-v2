...
(function() {
    ...
    if( fromCurrency != toCurrency){
      fetch("https://"+exHost+"/latest?base="+fromCurrency+"&symbols="+toCurrency)
        ...
        .then((data) => {
          ...
          $('input[name=quote_'+quoteNumber+'_exchange_rate]').val(round(conversionNum,2).toFixed(2));
        }).catch(e => {
  ...
  const currencyFormatter = function(num,currencyTarget){
    let toCurrency = currencyTarget ? currencyTarget : $('input[name=country_origin_currency_selection]').val(),
  ...
  const setCountryOriginELS = function(){
    ...
    if(countryOrigin && countryOriginCurrency && dob){
       ...
       $('input[name=dob]').val(dob);
        ...
        $('.close-quote').hide();
        let quoteModal = new bootstrap.Modal('#quote-modal', {
          backdrop: 'static',
          keyboard: false
        });
  ...
  const setMarketPriceCode = function(){
    let schoolName = $('input[name=school_selection]').val(),
        ...
        quoteObj = JSON.parse(localStorage.getItem(quoteJsonName));
    ...
    for (var i = 0; i < quoteJson.length; i++) {
      if(quoteJson[i].campus_country == campusCountry && quoteJson[i].school == schoolName){
        ...
        quoteObj.quote_array[quoteVal].market_price_code = quoteJson[i].region_code;
        quoteObj.quote_array[quoteVal].market_price_id = quoteJson[i].region_id;
      }
    }
  ...
  //return age at startDate of program or accommodation
  const returnAge = function(dob,startDate){
    let diff = startDate - dob,
  ...
  const initiateLocalJson = function(){
    let countryOrigin = $('input[name=country_origin_selection]').val(),
        ...
        schoolId = $('input[name=school_id_selection]').val(),
  ...
  const initEditQuote = function(editNum){
     let quoteObj = JSON.parse(localStorage.getItem("quote-obj")),
      ...
      $('input[name=campus_id_selection]').val(quoteObjQuote.campus_id);
      ...
      $('input[name=quote_edit_original_campus_id]').val(campusIdSelection);
      ...
      if(quoteObjQuote.promo.promo_code_enabled){
        $('input[name=promo_code]').val(quoteObjQuote.promo.promo_code);
      }
  ...
  $("form#pricing-quote").on('click','input[name=is_agent]',function(){
    let isAgentVal = $(this).val(),
        agentFormEl = '.agent-form-container';
    if(isAgentVal == 'Yes'){
      $(agentFormEl).css('display','block');
    }else{
      $(agentFormEl).css('display','none');
      $('input',agentFormEl).val('');
    }
  });
