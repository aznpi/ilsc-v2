...
const redirectToQuoteSummary = function () {
  ...
  let quoteSlide = $(".quote-step-slide .slick-slide").index(
    $("#quote-summary")
  );
...
const updateCampusSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    campusSelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
  ).attr("data-selected-campus");
    campusIdSelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
  ).attr("data-selected-campus-id");
    campusCountrySelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
  ).attr("data-country");
    campusCurrencySelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
    ).attr("data-selected-campus-currency"),
    programSelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
    ).attr("data-program-area"),
  dataDisableResidenceCurrency = $(
    ".campus-selection-container .campus-select[data-selected=true]"
  ).attr("data-disable-residence-currency");
...
const disableCampusDrop = function () {
  ...
  $(".campus-selection-container button.next")
    .removeClass("disable")
    .addClass("enable");
};

const setCheckInCalendar = function (country) {
  $(function () {
...
$(".campus-selection-container .dropdown-menu li.campus-name").on(
  "click",
  function () {
    let getValue = $(this).text(),
      getId = $(this).attr("data-campus-id"),
      getResidenceDisableCurrency = $(this).attr(
        "data-disable-residence-currency"
      ),
      getCountryValue = $(this)
        .parent()
        .siblings("button.campus-select")
    ...
    $(this)
      ...
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-selected-campus", getValue);
    $(this)
      .parent()
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-selected-campus-id", getId);
    $(this)
      .parent()
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-selected-campus-currency", getCampusCurrency);
    $(this)
      .parent()
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-disable-residence-currency", getResidenceDisableCurrency);
...
$(".campus-selection-container .dropdown button.direct-link").on(
  "click",
  function () {
    let getValue = $(this).find("span.selected-text").text(),
      getId = $(this).attr("data-selected-campus-id"),
...
$("#program-list-container").on("click", "input[type=radio]", function () {
  ...
  $(".program-list-selection-container button.next")
    .removeClass("disable")
    .addClass("enable");
});
...
const getProgramsList = function () {
  let isEdit = $("input[name=quote_edit]").val(),
    ...
    queryParam =
      "&campus__in=" +
      campusId +
      "&show_quote_tool__eq=1&program_type__in=" +
      programName +
      "&sort=program_name",
        ...
        if (dataObject.length > 0) {
          for (let i = 0; i < dataObject.length; i++) {
            ...
            programDescription = programDescription
              ? programDescription.replace(/['\u2019]/g, "&rsquo;")
              : "";
            ...
            radioHtml =
              "<div class='program-option'><input type='radio' autocomplete='off' class='program-radio' name='program_radio_selection' id='" +
              programId +
              "' data-category='" +
              programCategory +
              "' data-description='" +
          ...
          radioHtml =
            "<div class='text-center'><h3>No Programs Available at this time.</h3></div>";
          $("#program-list-container").append(radioHtml);
        }
      })
      .fail(function () {
...
$("#program-price-list-container").on(
  "click",
  "input[type=radio]",
  function () {
    $(".program-price-list-selection-container button.next")
      .removeClass("disable")
      .addClass("enable");
  }
);
...
const printProgramHeaderTitle = function () {
  ...
  $(".step-container.program-container .program-title.step-title h3").text(
    programTitle
  );
  $(
    ".program-price-list-selection-container.step-container.program-container .program-title.step-title .p-class"
  ).html(programDesc);
};
        ...
        .then(function () {
          if (dataObject1.length > 0) {
            programPriceArray = dataObject1.map((std) => ({
              ...
              family_additional_enable:
                std.values.enable_additional_family_members,
            }));
          }
          ...
          if (programPriceArray.length > 0) {
            ...
            radioHtml =
              "<div class='text-center'><h3>No Classes for this Program Available at this time.</h3></div>";
...
const getProgramsPriceListLowest = function () {
  let marketPriceCode = $("input[name=market_price_code]").val(),
    ...
    queryParam =
      ...
      "&programs_available__in=" +
      programName +
      "&is_program_price__eq=1&enable__eq=1&sort=price_current",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam,
    programPriceArray = [];
    ...
    success: function (data) {
      ...
      for (let i = 0; i < dataObject.length; i++) {
        ...
        programPriceArray.push({
          ...
          family_additional_enable:
            dataObject[i].values.enable_additional_family_members,
        });
      }
...
const printStandardLowestPriceProgramList = function (listArray) {
  let isEdit = $("input[name=quote_edit]").val(),
    selectedGroupId = $(
      "input[name=quote_edit_original_program_schedule_selection]"
    ).val(),
  ...
  if (listArray.length > 0) {
    ...
    $(".step-content-container .program-title.step-title").html(
    ...
    for (let i = 0; i < listArray.length; i++) {
      ...
      radioHtml =
        "<div class='program-option'><input type='radio' autocomplete='off' class='program-option' name='program_price_selection' id='" +
        groupId +
        "' value='" +
        groupId +
        "' data-program-name='" +
        ...
        "' data-item-name='" +
        itemName +
        "' data-family-additional-enable='" +
        familyAdditionalEnable +
        "' data-price-format='" +
        priceFormat +
        "' data-price='" +
        priceConversion +
        ...
        "/><label for='" +
        groupId +
        "'><div class='program-name float-left'>" +
        scheduleName +
        " - " +
        itemName +
        "</div><div class='text-right float-right'><span class='price-format'>" +
        priceFormat +
        " from</span><span class='price-text'>" +
        price +
        "</span></div></label></div>";
      $("#program-price-list-container").append(radioHtml);
    }
  ...
  for (let y = 0; y < listArrayP.length; y++) {
    ...
    $.ajax({
      ...
      success: function (dataP) {
        ...
        let priceLocal =
          JSON.parse(localStorage.getItem("program-price")) || [];
...
const printMarketLowestPriceProgramList = function () {
  ...
  if (retrievedJson.length > 0) {
    ...
    for (let i = 0; i < retrievedJson.length; i++) {
      ...
      priceConversion = round(xConversion(retrievedJson[i].price), 1).toFixed(
        2
      );
      price = currencyFormatter(priceConversion);
  ...
  if (listArray.length > 0) {
    ...
    for (let i = 0; i < listArray.length; i++) {
      ...
      if (listArray[i].price_additional) {
        priceAdditionalConversion = round(
          xConversion(listArray[i].price_additional),
          1
        ).toFixed(2);
      }
      if (listArray[i].price_additional_2) {
        priceAdditionalConversion2 = round(
          xConversion(listArray[i].price_additional_2),
          1
        ).toFixed(2);
      }
      price = currencyFormatter(priceConversion);
      ...
      radioHtml =
        ...
        programName +
        "' data-price-addition='" +
        priceAdditionalConversion +
        "' data-price-addition-2='" +
        priceAdditionalConversion2 +
        "' data-duration='" +
        duration +
        "' data-item-name='" +
        scheduleName +
        ...
        "</div><div class='price-container text-right float-right'><span class='price-text'>" +
        price +
        "</span></div></label></div>";
...
const updatePriceSelection = function () {
  $("#program-price-list-container >div.program-option").click(function () {
    let programId = $("input[name=program_price_selection]", this).val(),
      stdPrice = $("input[name=program_price_selection]", this).attr(
        "data-price"
      ),
      price = $("input[name=program_price_selection]", this).attr("data-price"),
      programName = $("input[name=program_price_selection]", this).attr(
        "data-program-name"
      ),
      programDescription = $("input[name=program_price_selection]", this).attr(
        "data-description"
      ),
      itemName = $("input[name=program_price_selection]", this).attr(
        "data-item-name"
      ),
      duration = $("input[name=program_price_selection]", this).attr(
        "data-duration"
      ),
      programCategory = $("input[name=program_category]").val(),
      additionalPrice = $("input[name=program_price_selection]", this).attr(
        "data-price-addition"
      ),
      additionalPrice2 = $("input[name=program_price_selection]", this).attr(
        "data-price-addition-2"
      );
...
const updateProgramSelectionTotalPrice = function () {
  ...
  quoteJson.quote_array[objPosition].program.material_fee =
    parseFloat(programMaterialFee);
  quoteJson.quote_array[objPosition].program.option_schedule =
    programOptionSchedule;
  quoteJson.quote_array[objPosition].program.option_schedule_description =
    programOptionDescription;
  quoteJson.quote_array[objPosition].program.program_total_price =
    parseFloat(programTotalPrice);
  quoteJson.quote_array[objPosition].program.program_total_standard_price =
    parseFloat(programStandardPrice);
  quoteJson.quote_array[objPosition].program.program_id = parseFloat(programId);
  ...
  if (currentVal != optionVal) {
    if (currentOption == "month") {
      if ($(".start-date-selection-container .selectDay").hasClass("active")) {
        ...
        $(".start-date-selection-container .selectDay .selectBox__value")
          .removeClass("active")
          .addClass("disabled")
          .attr("data-selected", "")
          .text("Day");
      } else {
        $(".start-date-selection-container .selectDay")
          .addClass("active")
          .removeClass("disabled");
      }
    } else if (currentOption == "year") {
      if (
        $(".start-date-selection-container .selectMonth").hasClass("active")
      ) {
        $(".start-date-selection-container .selectMonth").removeClass("show");
        $(".start-date-selection-container .selectMonth .selectBox__value")
          .removeClass("active")
          .addClass("disabled")
          .attr("data-selected", "")
          .text("Month");
...
$(document).on(
  "click",
  ".start-date-selection-container .select-container .selectBox.active",
  function (e) {
    ...
    let yearValue = $(
      ...
      monthValue = $(
        ".start-date-selection-container .selectBox .selectBox__value[data-option=month]"
      ).attr("data-selected");

    if (yearValue != "" && monthValue != "") {
      $(".start-date-selection-container button.next")
        .removeClass("disable")
        .addClass("enable");
    } else {
...
$(document).on(
  "click",
  ".accommodation-price-selection-container .select-container .selectBox.active",
  function (e) {
    $(this).toggleClass("show");
    let dropdownItem = e.target;
...
$(document).on(
  "click",
  ".family-add-selection-container .select-container .selectBox.active",
  function (e) {
    $(this).toggleClass("show");
    let dropdownItem = e.target,
      container = $(this).find(".selectBox__value");
    ...
    let childValue = $(
        ".family-add-selection-container .selectBox .selectBox__value[data-option=child]"
      ).attr("data-selected"),
      adultValue = $(
        ".family-add-selection-container .selectBox .selectBox__value[data-option=adult]"
      ).attr("data-selected");
    ...
    if (childValue != "" || adultValue != "") {
      $(".family-add-selection-container button.next")
        .removeClass("disable")
        .addClass("enable");
    }
  }
...
$(document).on(
  "click",
  ".start-date-selection-container .select-container .selectBox.active .dropdown-item",
  function (e) {
    e.preventDefault();
...
const printStartDateSelection = function () {
  ...
  let groupId = $("input[name=program_schedule_price_id_selection]").val(),
    ...
    priceFormat = $("input[name=program_price_selection]:checked").attr(
      "data-price-format"
    ),
  ...
  if (priceFormat) {
    priceFormatHtml =
      "<span class='price-format'>" + priceFormat + " from</span>";
  }

  $(
    ".step-container.program-container .program-title.step-title h5.program-item"
  ).text(programName);

  radioHtml =
    "<div class='program-option'><input type='radio' autocomplete='off' class='program-option' name='program_start_date_selection' id='" +
    groupId +
    "' value='" +
    groupId +
    "' data-price-format='" +
    ...
    programName +
    " - " +
    programScheduleName.toUpperCase() +
    "</div><div class='price-container text-right float-right'>" +
    priceFormatHtml +
...
const printDurationSelection = function () {
  ...
  for (let z = 1; z < 53; z++) {
    dropHtml =
      dropHtml +
      '<a href="#" class="dropdown-item" data-item="' +
      z +
      '" tabindex="0">' +
      z +
      " Weeks</a>";
  ...
  container =
    "<div class='select-title'><h5>Please choose the <span class='bold'>Duration</span></h5></div><div class='select-container'><div id='duration-select' class='select-option'><div class='selectBox selectDuration active'><div class='selectBox__value' data-option='duration' data-selected=''>Duration</div><div class='dropdown-menu'>" +
    dropHtml +
    "</div></div></div></div>";
...
const getStartDates = function () {
  ...
  if (getDates) {
    ...
    localStorage.setItem(
      "start-dates",
      JSON.stringify(groupBy(yearMonthArray, "year"))
    );
...
const resetPrintDates = function () {
  let yearVal = $(
      ".date-container #year-select .selectBox .selectBox_value.active"
    ).attr("data-selected"),
    monthVal = $(
      ".date-container #month-select .selectBox .selectBox_value.active"
    ).attr("data-selected");

  isEdit = $("input[name=quote_edit]").val();

  if (yearVal != "" && monthVal != "") {
    $(".start-date-selection-container .selectYear .selectBox__value")
      .attr("data-selected", "")
      .text("Year");
    $(".start-date-selection-container .selectMonth")
      .removeClass("active")
    ...
    if (isEdit === "true") {
      $("button.next[data-step=start-date-select]")
        .removeClass("enable")
        .addClass("disable");
    }
  }
...
const printDates = function (timeVal, dataItem, dataYear) {
  ...
  if (Object.keys(startDateArray).length > 0) {
    ...
    $(
      ".start-date-selection-container.program-container .no-start-dates"
    ).remove();
    if (timeVal == "month") {
      ...
      for (let year in startDateArray) {
        if (year == dataItem) {
          for (let month in startDateArray[year][0].month) {
            ...
            for (
              let i = 0;
              i < startDateArray[year][0].month[month].length;
              i++
            ) {
              day = startDateArray[year][0].month[month][i].day;
              monthHtml =
                '<a href="#" class="dropdown-item" data-year="' +
                year +
                '"data-parent="month" data-item="' +
                month +
                '" data-day="' +
                day +
                '" tabindex="0">' +
                ...
                "</a>";
              $(
                ".date-container #month-select .selectBox .dropdown-menu"
              ).append(monthHtml);
            }
          }
        }
      ...
      for (let year in startDateArray) {
        yearHtml =
          '<a href="#" class="dropdown-item" data-parent="year" data-item="' +
          year +
          '" tabindex="0">' +
          year +
          "</a>";
        $(".date-container #year-select .selectBox .dropdown-menu").append(
          yearHtml
        );
      }
    }
    ...
    if (
      ...
      $(".start-date-selection-container.program-container").append(
        '<h4 class="center no-start-dates">No Start Dates Available</h4>'
      );
    }
  }
};
...
const updateJsonStartDatePrice = function () {
  let editNum = $("input[name=quote_number]").val(),
    ...
    quoteObject = localStorage.getItem(quoteJsonName),
    quoteJson = JSON.parse(quoteObject);
  programSchedulePrice = $(
    "input[name=program_schedule_price_selection]"
  ).val();
  programScheduleStandardPrice = $(
    "input[name=program_schedule_standard_price_selection]"
  ).val();

  for (var i = 0; i < quoteJson.quote_array.length; i++) {
    if (i === objPosition) {
      quoteJson.quote_array[i].program.program_total_price =
        programSchedulePrice;
      quoteJson.quote_array[i].program.program_total_standard_price =
        programScheduleStandardPrice;
      break;
    }
  }
...
const setProgramPriceDuration = function (listArray) {
  let durationNum = parseInt(
      $(".selectBox .selectBox__value[data-option=duration]").attr(
        "data-selected"
      )
    ),
    marketPriceCode = $("input[name=market_price_code]").val(),
  ...
  for (let i = 0; i < listArray.length; i++) {
    ...
    if (durationNum >= minVal && durationNum <= maxVal) {
      ...
      xStandardPrice = parseFloat(listArray[i].priceStandard) * durationNum;
      xStandardPrice = round(xConversion(xStandardPrice), 1).toFixed(2);

      $("input[name=program_schedule_selection]").val(
        listArray[i].scheduleName + "+" + listArray[i].itemName
      );
      $("input[name=program_schedule_price_selection]").val(xPrice);
      $("input[name=program_schedule_standard_price_selection]").val(
        xStandardPrice
      );
...
const setProgramMarketPriceDuration = function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (let i = 0; i < dataObject.length; i++) {
        price = parseFloat(dataObject[i].values.price) * duration;
...
const updateStartDateSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    monthValue = $(".selectBox .selectBox__value[data-option=month]").attr(
      "data-selected"
    ),
...
const setOtherProgramFees = function (feeArray) {
  let editNum = $("input[name=quote_number]").val(),
    ...
    minMonthHighSeason = new Date(
      $("input[name=high_season_min_date_current_year]").val()
    ),
    maxMonthHighSeason = new Date(
      $("input[name=high_season_max_date_current_year]").val()
    ),
  ...
  for (let i = 0; i < setFeeArray.length; i++) {
    if (setFeeArray[i].feeType == "Registration Fees") {
      quoteJson.quote_array[objPosition].program.registration_fee = round(
        xConversion(setFeeArray[i].feePrice),
        1
      ).toFixed(2);
      quoteJson.quote_array[objPosition].program.registration_description =
        setFeeArray[i].feeDescription;
    }
    if (setFeeArray[i].feeType == "Material Fees") {
      ...
      if (setFeeArray[i].priceType == "Per week") {
        ...
        isMaxFee = setFeeArray[i].maxFee ? true : false;

        if (isMaxFee && feeVal > setFeeArray[i].maxFee) {
          feeVal = setFeeArray[i].maxFee;
        }
      }
      quoteJson.quote_array[objPosition].program.material_fee = round(
        xConversion(feeVal),
        1
      ).toFixed(2);
    }
    if (setFeeArray[i].feeType == "Additional Fees") {
      ...
      quoteJson.quote_array[objPosition].program.additional_fee = round(
        xConversion(addFeeVal),
        1
      ).toFixed(2);
      quoteJson.quote_array[objPosition].program.additional_fee_description =
        setFeeArray[i].feeDescription;
    }
  }
  localStorage.setItem(quoteJsonName, JSON.stringify(quoteJson));
...
const getOtherProgramFees = function () {
  let campus = $("input[name=campus_selection]").val(),
    ...
    queryParam =
      "&campus_location__in=" +
      campusId +
      "&category__eq=Other Program Fees&is_program_price__eq=0&programs_available__in=" +
      programName +
      "&enable__eq=1&sort=-price_current",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam,
    feeArray = [];
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (let i = 0; i < dataObject.length; i++) {
        rangeVal = dataObject[i].values.weeks
          ? dataObject[i].values["weeks"].name
              .replace(/\s/g, "")
              .replace("Weeks", "")
          : "";
...
//start Family option
$(
  ".family-option-selection-container .family-option-selection-container .option-select.program-option"
  ...
  $(".family-option-selection-container button.next")
    .removeClass("disable")
    .addClass("enable");
});
...
const removeFamilyInfo = function () {
  ...
  quoteJson.quote_array[
    quoteNum
  ].accommodation.family_additional_adult_total_price = 0;
  quoteJson.quote_array[
    quoteNum
  ].accommodation.family_additional_child_total_price = 0;
...
const redirectFamilyStep = function () {
  let programCategory = $("input[name=program_category]").val(),
    ...
    ).attr("data-family-additional-enable"),
    isEdit = $("input[name=quote_edit]").val();
  ...
  if (familyAdditionalEnable == "1") {
    ...
    subText =
      programCategory == "Family Camps"
        ? "Make it a Family Winter Experience by adding additional family members to your adventure. Package pricing is for 1 Child. Please indicate how many additional children aged 9-17, and how many adults you would like to add."
        : programSelection.includes("Junior Winter Experience")
        ? "Make it a Family Winter Experience by adding additional family members to your adventure. Package pricing is for 1 Child. Please indicate how many additional children aged 9-17, and how many adults you would like to add."
        : "Please indicate how many additional children aged 9-17, and how many adults you would like to add.";

    $(
      "#family-option-select p.sub-text-description, #family-add-select p.sub-text-description"
    ).html(subText);
...
const printFamilyAdd = function () {
  for (let i = 0; i < 6; i++) {
    childHtml =
      '<a href="#" class="dropdown-item" data-parent="child" data-item="' +
      i +
      '" tabindex="0">' +
      i +
      "</a>";
    adultHtml =
      '<a href="#" class="dropdown-item" data-parent="adult" data-item="' +
      i +
      '" tabindex="0">' +
      i +
      "</a>";
    $(
      ".family-option-add-selection-container #child-select .dropdown-menu"
    ).append(childHtml);
    $(
      ".family-option-add-selection-container #adult-select .dropdown-menu"
    ).append(adultHtml);
  }
};
  ...
  if (familyEnable == "true") {
    for (var i = 0; i < quoteJson.quote_array.length; i++) {
      if (i === objPosition) {
        ...
        quoteJson.quote_array[i].accommodation.family_additional_child =
          parseInt(childAdd);
        quoteJson.quote_array[i].accommodation.family_additional_adult =
          parseInt(adultAdd);
        break;
      }
    }
...
$("#schedule-price-list-container").on(
  "click",
  "input[type=radio]",
  function () {
    ...
    $(".schedule-price-list-selection-container button.next")
      .removeClass("disable")
      .addClass("enable");
  }
);
...
const getProgramScheduleList = function () {
  ...
  $.get(api_url)
    ...
    .fail(function () {
      radioHtml =
        "<div class='text-center'><h3>No Schedule for this Program Available at this time.</h3></div>";
...
const getMarketProgramPrice = function (listArray) {
  ...
  for (let y = 0; y < listArray.length; y++) {
    ...
    queryPriceParam =
      "&price_list_code__in=" + priceId + "&market_region__in=" + marketPriceId;
    let api_price_url =
      apiUrl +
      tableMarketPriceId +
      "/rows?portalId=" +
    ...
    $.ajax({
      ...
      success: function (dataP) {
        let dataPriceObject = dataP.results;
        let priceLocal =
          JSON.parse(localStorage.getItem("schedule-price")) || [];
        for (let x = 0; x < dataPriceObject.length; x++) {
...
const printStandardProgramPrice = function (listArray) {
  ...
  for (let i = 0; i < listArray.length; i++) {
    ...
    radioHtml =
      radioHtml +
      "<div class='program-option'><input type='radio' class='program-option' name='schedule_price_selection' id='" +
      priceId +
      "' value='" +
      priceId +
      "' data-schedule-name='" +
...
const setTotalPrice = function () {
  ...
  if (isFamilyEnable === "true") {
    ...
    quoteObj.quote_array[editNum].accommodation.family_additional_child =
      parseInt(childVal);
    quoteObj.quote_array[editNum].accommodation.family_additional_adult =
      parseInt(adultVal);
    quoteObj.quote_array[
      editNum
    ].accommodation.family_additional_adult_total_price = totalAdultPrice;
    quoteObj.quote_array[
      editNum
    ].accommodation.family_additional_child_total_price = totalChildPrice;
  } else {
    quoteObj.quote_array[editNum].program.duration = durationSelected;
  }

  totalAddPrice = parseFloat(totalAddPrice);

  schedulePriceSelected = parseFloat(
    quoteObj.quote_array[editNum].program.program_total_price
  );
  additionalFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].program.additional_fee
  );
  materialFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].program.material_fee
  );
  registrationFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].program.registration_fee
  );
  accommodationTotalSelected = parseFloat(
    quoteObj.quote_array[editNum].accommodation.accommodation_total_price
  );
  airportTransferFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].accommodation.airport_transfer_total_price
  );
  accommodationPlacementFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].accommodation.placement_fee
  );
  accommodationCleaningFeeSelected = parseFloat(
    quoteObj.quote_array[editNum].accommodation.cleaning_fee
  );
  totalPrice =
    schedulePriceSelected +
    materialFeeSelected +
    additionalFeeSelected +
    registrationFeeSelected +
    accommodationTotalSelected +
    airportTransferFeeSelected +
    accommodationPlacementFeeSelected +
    accommodationPlacementFeeSelected +
    totalAddPrice;
  ...
  if (quoteObj.quote_array[editNum].promo.promo_auto_enabled) {
    promoAutoValueArray =
      quoteObj.quote_array[editNum].promo.promo_auto_value.split("|");
    promoAutoCategory =
      quoteObj.quote_array[editNum].promo.promo_auto_category.split("|");
    promoAutoDiscountType =
      quoteObj.quote_array[editNum].promo.promo_auto_discount_type.split("|");

    promoAutoDiscount = 0;

    for (let i = 0; i < promoAutoValueArray.length; i++) {
      promoValueInt = convertPercentDollarVal(
        promoAutoValueArray[i],
        promoAutoCategory[i],
        promoAutoDiscountType[i],
        editNum
      );
...
const printQuotesHTML = function () {
  let destinationCurrency = $(
      "input[name=destination_currency_selection]"
    ).val(),
    dataDisableResidenceCurrency = $(
      ".campus-selection-container .campus-select[data-selected=true]"
    ).attr("data-disable-residence-currency"),
    queryString = window.location.search,
    urlParams = new URLSearchParams(queryString),
    countryOriginCurrencyParam = urlParams.get("currency");

  $(".step-breadcrumb-container").hide();
  $(".x-convert-container .btn[data-origin-currency]")
    .addClass("active")
    .addClass("btn-orange-outline")
    .removeClass("btn-black-outline");
  $(".x-convert-container .btn[data-destination-currency]")
    .removeClass("active")
    .removeClass("btn-orange-outline")
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        priceProgramVal = parseFloat(
          quoteObj.quote_array[i].program.program_total_price
        );
        priceStandardVal = parseFloat(
          quoteObj.quote_array[i].program.program_total_standard_price
        );
        priceRegistrationVal = parseFloat(
          quoteObj.quote_array[i].program.registration_fee
        );
        priceAdditionalVal = parseFloat(
          quoteObj.quote_array[i].program.additional_fee
        );
        priceMaterialVal = parseFloat(
          quoteObj.quote_array[i].program.material_fee
        );
        priceAccommodationVal = parseFloat(
          quoteObj.quote_array[i].accommodation.accommodation_total_price
        );
        priceAirportVal = parseFloat(
          quoteObj.quote_array[i].accommodation.airport_transfer_total_price
        );
        priceAccommodationPlacementFeeVal = parseFloat(
          quoteObj.quote_array[i].accommodation.placement_fee
        );
        priceAccommodationCleaningFeeVal = parseFloat(
          quoteObj.quote_array[i].accommodation.cleaning_fee
        );
        priceQuoteTotalVal = parseFloat(
          quoteObj.quote_array[i].quote_total_price
        );
        familyEnable = quoteObj.quote_array[i].accommodation.family_enable;
        addChildVal = parseFloat(
          quoteObj.quote_array[i].accommodation.family_additional_child
        );
        addAdultVal = parseFloat(
          quoteObj.quote_array[i].accommodation.family_additional_adult
        );
        addChildPrice = parseFloat(
          quoteObj.quote_array[i].accommodation
            .family_additional_child_total_price
        );
        addAdultPrice = parseFloat(
          quoteObj.quote_array[i].accommodation
            .family_additional_adult_total_price
        );
        ...
        promoCodeHtml = "";
        printPromo =
          quoteObj.quote_array[i].promo.promo_auto_enabled ||
          quoteObj.quote_array[i].promo.promo_code_enabled
            ? true
            : false;
        ...
        if (
          ...
          startDateObj <= exceptionEndDate
        ) {
          if (
            quoteObj.quote_array[i].program.option_schedule ==
            exceptionProgramSchedule
          ) {
        ...
        if (printPromo) {
          ...
          if (quoteObj.quote_array[i].promo.promo_auto_enabled) {
            promoCode =
              quoteObj.quote_array[i].promo.promo_auto_code.split("|");
            promoCategory =
              quoteObj.quote_array[i].promo.promo_auto_category.split("|");
            promoName =
              quoteObj.quote_array[i].promo.promo_auto_code_name.split("|");
            promoDescription =
              quoteObj.quote_array[i].promo.promo_auto_description.split("|");
            promoDiscountType =
              quoteObj.quote_array[i].promo.promo_auto_discount_type.split("|");
            promoType =
              quoteObj.quote_array[i].promo.promo_auto_type.split("|");
            promoValue =
              quoteObj.quote_array[i].promo.promo_auto_value.split("|");
          ...
          if (promoAutoArray.length > 0) {
            ...
            for (let z = 0; z < promoAutoArray.length; z++) {
              promoValueInt = convertPercentDollarVal(
                promoAutoArray[z].promoValue,
                promoAutoArray[z].category,
                promoAutoArray[z].discountType,
                i
              );

              promoLineItem +=
                "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>" +
                promoAutoArray[z].promoName +
                "</span></div><div class='item-notes'>" +
                promoAutoArray[z].description +
                "</div></div><div class='item-price bold promo-auto-total promo-auto-total-" +
                z +
                "'>" +
                currencyFormatter(-promoValueInt) +
                "</div></div></div>";
              promoAutoTotalVal += parseFloat(promoValueInt);
            }

            promoAutoHtml =
              "<div class='quote-box promo-box-container promo-auto-box'>" +
              promoLineItem +
              "</div>";
          }
          ...
          if (quoteObj.quote_array[i].promo.promo_code_enabled) {
            promoValueInt = convertPercentDollarVal(
              quoteObj.quote_array[i].promo.promo_code_value,
              quoteObj.quote_array[i].promo.promo_code_category,
              quoteObj.quote_array[i].promo.promo_code_discount_type,
              i
            );

            promoCodeHtml =
              "<div class='quote-box promo-box-container'><div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>" +
              quoteObj.quote_array[i].promo.promo_code_name +
              "</span></div><div class='item-notes'>" +
              quoteObj.quote_array[i].promo.promo_code_description +
              "</div></div><div class='item-price bold promo-code-total'>" +
              currencyFormatter(-promoValueInt) +
              "</div></div></div></div>";
          }
        }
        ...
        if (priceProgramVal < priceStandardVal) {
          ...
          discountHtml =
            "<span class='discount-txt strike discount-program-price'>" +
            currencyFormatter(priceStandardVal) +
            "</span>";
          discountTotalHtml =
            "<div class='total-discount-line'><div class='item-description total-title discount-title-price'>Discount Total</div><div class='item-price'><span class='discount-txt discount-total-price'>" +
            currencyFormatter(-discountVal) +
            "</span></div></div>";
        }

        schoolHtml =
          "<div class='program-type'><span class='header'>Program</span><br><span class='result'>" +
          programSchoolName +
          "</div>";
        campusHtml =
          "<div class='program-type'><span class='header'>Campus</span><br><span class='result'>" +
          quoteObj.quote_array[i].campus +
          ", " +
        ...
        scheduleProgramHtml =
          "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>" +
          quoteObj.quote_array[i].program.option_program +
          "</span></div><div class='item-notes'>Start: " +
          ...
          currencyFormatter(priceProgramVal) +
          "</span></div></div></div>";

        if (priceRegistrationVal > 0) {
          registrationHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Registration Fee</span></div><div class='item-notes'>" +
            quoteObj.quote_array[i].program.registration_description +
            "</div></div><div class='item-price'><span class='bold registration-fee'>" +
            currencyFormatter(priceRegistrationVal) +
            "</span></div></div></div>";
        }
        if (priceAdditionalVal > 0) {
          additionalHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Additional Fee</span></div><div class='item-notes'>" +
            quoteObj.quote_array[i].program.additional_fee_description +
            "</div></div><div class='item-price'><span class='bold additional-fee'>" +
            currencyFormatter(priceAdditionalVal) +
            "</span></div></div></div>";
        }
        ...
        if (descriptionVal != "") {
          includesHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>INCLUDES</span><div>" +
            descriptionVal +
            "</div></div></div></div></div>";
        }
        ...
        if (familyEnable) {
          if (addChildVal > 0) {
            ...
            childHtml =
              "<div class='quote-box'><div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Additional Child</span><br><ul><li>" +
              addChildVal +
              " " +
              childString +
              "</li><li>" +
              quoteObj.quote_array[i].program.option_schedule +
              "</li></ul></div></div><div class='item-price'><span class='bold child-fee'>" +
              currencyFormatter(addChildPrice) +
          ...
          if (addAdultVal > 0) {
            ...
            adultHtml =
              "<div class='quote-box'><div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Additional Adult</span><br><ul><li>" +
              addAdultVal +
              " " +
              adultString +
              "</li><li>" +
              quoteObj.quote_array[i].program.option_schedule +
              "</li></ul></div></div><div class='item-price'><span class='bold adult-fee'>" +
              currencyFormatter(addAdultPrice) +
              "</span></div></div></div></div>";
          }
        }
        ...
        if (priceAccommodationVal > 0) {
          optionName =
            quoteObj.quote_array[i].accommodation.option_accommodation;
          accommodationPriceHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>" +
            optionName +
            "<br>(" +
            quoteObj.quote_array[i].accommodation.length_of_stay +
            " Weeks)</span></div><div class='item-notes'>Check-in Date: " +
            quoteObj.quote_array[i].accommodation.check_in_date +
            "</div></div><div class='item-price'><span class='bold accommodation-price'>" +
            currencyFormatter(priceAccommodationVal) +
            "</span></div></div></div>";
          airportPriceHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Airport Transfer Fee</span></div><div class='item-notes'>" +
            quoteObj.quote_array[i].accommodation.option_airport_transfer +
            "</div></div><div class='item-price'><span class='bold airport-fee'>" +
            currencyFormatter(priceAirportVal) +
            "</span></div></div></div>";
          placementFeePriceHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Accommodation Administration Fee</span></div></div><div class='item-price'><span class='bold placement-fee'>" +
            currencyFormatter(priceAccommodationPlacementFeeVal) +
            "</span></div></div></div>";
          if (priceAccommodationCleaningFeeVal > 0) {
            cleaningFeePriceHtml =
              "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Exit Cleaning Fee</span></div></div><div class='item-price'><span class='bold cleaning-fee'>" +
              currencyFormatter(priceAccommodationCleaningFeeVal) +
              "</span></div></div></div>";
          }
          accommodationHtml =
            "<div class='quote-accommodation quote-box'>" +
            accommodationPriceHtml +
            airportPriceHtml +
            placementFeePriceHtml +
            cleaningFeePriceHtml +
            "</div>";
          accommodationNoteHtml =
            accommodationNoteVal != "none" || accommodationNoteVal !== ""
        ...
        priceNoteText =
          "<sup>*</sup>The prices shown are not valid for government-sponsored business.";
        priceNoteHtml =
          "<div class='center footnote'>" + priceNoteText + "</div>";

        totalPriceHtml =
          "<div class='line-item'>" +
          discountTotalHtml +
          "<div><div class='item-description total-title bold'>TOTAL</div><div class='item-price bold total-price'>" +
          currencyFormatter(priceQuoteTotalVal) +
          "</div></div></div>";
        quoteHTML =
          "<div class='col col-md-6 col-quote col-quote-" +
          quoteNum +
          "'><div class='quote-title'>Quote " +
          quoteNum +
          "</div><div class='quote-select-container'><div class='quote-items'><div class='quote-location'>" +
          schoolHtml +
          campusHtml +
          "</div><div class='quote-program-schedule quote-box'>" +
          scheduleProgramHtml +
          registrationHtml +
          additionalHtml +
          materialHtml +
          ...
          "</div>" +
          printExceptionHtml +
          "<div class='quote-box total-price-container'>" +
          totalPriceHtml +
          "</div>" +
          accommodationNoteHtml +
          priceNoteHtml +
          "</div><div class='promo-container'><label for='input-promo-code-" +
          quoteNum +
          "'>Enter Promo Code</label><div><input type='text' id='input-promo-code-" +
          quoteNum +
          "' class='input-promo-code' name='input-promo-code-" +
          quoteNum +
          "'><button data-quote='" +
          quoteNum +
          "' class='btn-promo-code btn-orange'>APPLY</button></div><div id='promo-feedback-" +
          quoteNum +
          "' class='promo-feedback'></div></div><div><div class='btn-container'><button type='button' class='btn btn-orange-outline bold quote-edit' data-edit-num='" +
          quoteNum +
          "' tabindex='0'>Edit</button><button type='button' class='btn btn-orange-outline bold quote-remove' data-remove-num='" +
          quoteNum +
          "' tabindex='0'>Remove <span class='icon-remove'>&#8999;</span></button></div></div>";
        ...
        quoteHTML =
          ...
          quoteNum +
          " empty'><div class='quote-title'>Quote " +
          quoteNum +
          "</div><div class='new-quote' data-quote='" +
          quoteNum +
          "'><div class='create-icon center'><i class='fas fa-plus-circle'></i></div><h3 class='center'>Create a new quote</h3></div></div>";
        $("#quote-selection-container").append(quoteHTML);
      }
    }
  }
  ...
  setTimeout(getExchangeRate(), 300);
};

const printQuoteWidth = function () {
  if (
    !$(".col-quote-2").hasClass("empty") &&
    $(".col-quote-3").hasClass("empty")
  ) {
...
const convertCalc = function (num, conversion) {
  let conversionRate = $(
    "input[name=quote_" + quoteNum + "_exchange_rate]"
  ).val();
    ...
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        if (fromCurrency != toCurrency) {
          ...
          priceProgramVal = convertCalcFloat(
            quoteObj.quote_array[i].program.program_total_price,
            convert
          );
          ...
          priceStandardVal = convertCalcFloat(
            quoteObj.quote_array[i].program.program_total_standard_price,
            convert
          );
          ...
          priceRegistrationVal = convertCalcFloat(
            quoteObj.quote_array[i].program.registration_fee,
            convert
          );
          ...
          priceAdditionalVal = convertCalcFloat(
            quoteObj.quote_array[i].program.additional_fee,
            convert
          );
          ...
          priceMaterialVal = convertCalcFloat(
            quoteObj.quote_array[i].program.material_fee,
            convert
          );
          ...
          priceAccommodationVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.accommodation_total_price,
            convert
          );
          ...
          priceAdultVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.family_additional_adult_total_price,
            convert
          );
          priceAdultVal = round(priceAdultVal, 1).toFixed(2);
          quoteObj.quote_array[i].accommodation.family_additional_adult_total_price = priceAdultVal;

          priceChildVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.family_additional_child_total_price,
            convert
          );
          priceChildVal = round(priceChildVal, 1).toFixed(2);
          quoteObj.quote_array[i].accommodation.family_additional_child_total_price = priceChildVal;

          priceAirportVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.airport_transfer_total_price,
            convert
          );
          ...
          priceAccommodationPlacementFeeVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.placement_fee,
            convert
          );
          priceAccommodationPlacementFeeVal = round(
            priceAccommodationPlacementFeeVal,
            1
          ).toFixed(2);
          quoteObj.quote_array[i].accommodation.placement_fee =
            priceAccommodationPlacementFeeVal;

          priceAccommodationCleaningFeeVal = convertCalcFloat(
            quoteObj.quote_array[i].accommodation.cleaning_fee,
            convert
          );
          priceAccommodationCleaningFeeVal = round(
            priceAccommodationCleaningFeeVal,
            1
          ).toFixed(2);
          quoteObj.quote_array[i].accommodation.cleaning_fee =
            priceAccommodationCleaningFeeVal;

          priceQuoteTotalVal = convertCalcFloat(
            quoteObj.quote_array[i].quote_total_price,
            convert
          );
          ...
          promoAutoValStr =
            quoteObj.quote_array[i].promo.promo_auto_value.toString();
          promoAutoValArray = quoteObj.quote_array[i].promo.promo_auto_enabled
            ? promoAutoValStr.split("|")
            : false;
          promoAutoCatStr =
            quoteObj.quote_array[i].promo.promo_auto_category.toString();
          promoAutoCategoryArray = quoteObj.quote_array[i].promo
            .promo_auto_enabled
            ? promoAutoCatStr.split("|")
            : false;
          promoAutoTypeStr =
            quoteObj.quote_array[i].promo.promo_auto_discount_type.toString();
          promoAutoTypeArray = quoteObj.quote_array[i].promo.promo_auto_enabled
            ? promoAutoTypeStr.split("|")
            : false;

          promoCodeCategory = quoteObj.quote_array[i].promo.promo_code_category;
          promoCodeType =
            quoteObj.quote_array[i].promo.promo_code_discount_type;
          promoCodeTotal = quoteObj.quote_array[i].promo.promo_code_enabled
            ? parseInt(quoteObj.quote_array[i].promo.promo_code_value)
            : 0;

          if (quoteObj.quote_array[i].promo.promo_auto_enabled) {
            convertedPromoAutoValArray = [];
            for (let x = 0; x < promoAutoValArray.length; x++) {
              promoAutoVal = convertPercentDollarVal(
                promoAutoValArray[x],
                promoAutoCategoryArray[x],
                promoAutoTypeArray[x],
                i
              );
              ...
              $(".promo-auto-total-" + x, ".col-quote-" + quoteNum).html(
          ...
          if (quoteObj.quote_array[i].promo.promo_code_enabled) {
            promoCodeVal = convertPercentDollarVal(
              promoCodeTotal,
              promoCodeCategory,
              promoCodeType,
              i
            ...
            $(".promo-code-total", ".col-quote-" + quoteNum).html(
              currencyModFormatter(-promoCodeTotal, toCurrency)
            );
          ...
          if (priceProgramVal < priceStandardVal) {
            ...
            $(".col-quote-" + quoteNum + " span.discount-program-price").html(
              currencyModFormatter(priceStandardPrice, toCurrency)
            );
            $(".col-quote-" + quoteNum + " span.discount-total-price").html(
              currencyModFormatter(discountVal, toCurrency)
            );
          }

          $(".col-quote-" + quoteNum + " span.program-price").html(
            currencyModFormatter(priceProgramPrice, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.registration-fee").html(
            currencyModFormatter(priceRegistrationVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.additional-fee").html(
            currencyModFormatter(priceAdditionalVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.material-fee").html(
            currencyModFormatter(priceMaterialVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.accommodation-price").html(
            currencyModFormatter(priceAccommodationVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.airport-fee").html(
            currencyModFormatter(priceAirportVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.placement-fee").html(
            currencyModFormatter(priceAccommodationPlacementFeeVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.cleaning-fee").html(
            currencyModFormatter(priceAccommodationCleaningFeeVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.child-fee").html(
            currencyModFormatter(priceAdultVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " span.adult-fee").html(
            currencyModFormatter(priceChildVal, toCurrency)
          );
          $(".col-quote-" + quoteNum + " div.total-price").html(
            currencyModFormatter(priceQuoteTotalVal, toCurrency)
          );
        }
      }
    }
...
const sendEmailQuoteForm = function () {
  ...
  let quoteObj = JSON.parse(localStorage.getItem("quote-obj")),
    ...
    nationality = $(
      "form#pricing-quote select[name=nationality] option:selected"
    ).val(),
  ...
  let agentName = $(
      "form#pricing-quote input[name=quote_agent_company_name]"
    ).val(),
    agentFirstName = $(
      "form#pricing-quote input[name=quote_agent_first_name]"
    ).val(),
    agentLastName = $(
      "form#pricing-quote input[name=quote_agent_last_name]"
    ).val(),
  ...
  for (let i = 0; i < quoteObj.quote_array.length; i++) {
    ...
    programTotalStandardPriceInt = parseFloat(
      quoteObj.quote_array[i].program.program_total_standard_price
    );
    ...
    totalAmount = quoteObj.quote_array[i].promo.promo_code_enabled
      ? parseFloat(quoteObj.quote_array[i].quote_total_price) -
        parseFloat(quoteObj.quote_array[i].promo.promo_code_value)
      : parseFloat(quoteObj.quote_array[i].quote_total_price);

    totalPrice = totalAmount.toFixed(2);

    accommodationEnable =
      quoteObj.quote_array[i].accommodation.accommodation_enable;
    familyEnable = quoteObj.quote_array[i].accommodation.family_enable;

    familyAdultAddEnable = parseInt(
      quoteObj.quote_array[i].accommodation.family_additional_adult
    );
    familyChildAddEnable = parseInt(
      quoteObj.quote_array[i].accommodation.family_additional_child
    );
    familyChildPrice = parseFloat(
      quoteObj.quote_array[i].accommodation.family_additional_child_total_price
    );
    familyAdultPrice = parseFloat(
      quoteObj.quote_array[i].accommodation.family_additional_adult_total_price
    );
    ...
    showPromoAuto = quoteObj.quote_array[i].promo.promo_auto_enabled
      ? "revert"
      : "none";
    showPromoCode = quoteObj.quote_array[i].promo.promo_code_enabled
      ? "revert"
      : "none";
    ...
    promoAutoCatStr =
      quoteObj.quote_array[i].promo.promo_auto_category.toString();
    promoAutoCategoryArray = quoteObj.quote_array[i].promo.promo_auto_enabled
      ? promoAutoCatStr.split("|")
      : false;
    promoAutoTypeStr =
      quoteObj.quote_array[i].promo.promo_auto_discount_type.toString();
    ...
    if (quoteObj.quote_array[i].promo.promo_auto_enabled) {
      if (promoAutoValArr.length > 0) {
        for (let z = 0; z < promoAutoValArr.length; z++) {
          autoValInt = convertPercentDollarVal(
            promoAutoValArr[z],
            promoAutoCategoryArray[z],
            promoAutoTypeArray[z],
            i
          );

          autoVal = parseFloat(autoValInt);
          promoAutoTotalVal += parseFloat(autoVal);
          promoAutoValArray.push(currencyModFormatter(autoVal, showCurrency));
        }
        promoAutoTotalVal = promoAutoTotalVal;
      }
    }
    promoAutoValStr =
      promoAutoValArray.length > 0
        ? promoAutoValArray.toString().replace("00,", "|")
        : "n/a";
    ...
    promoCodeVal = quoteObj.quote_array[i].promo.promo_code_enabled
      ? convertPercentDollarVal(
          promoCodeTotal,
          promoCodeCategory,
          promoCodeType,
          i
      ...
      : 0;
    promoCodeVal = parseFloat(promoCodeVal);

    if (programTotalStandardPriceInt > programTotalPriceInt) {
      discountPrice =
        programTotalStandardPriceInt -
        programTotalPriceInt +
        promoAutoTotalVal +
        promoCodeVal;
    ...
    if (quoteObj.quote_array[i].quote_enabled) {
      formFieldsArray.push(
        ...
        },
        {
          objectTypeId: "0-1",
          name:
            "quote_" + quoteNum + "_program_schedule_standard_price_selection",
