...
const getProgramsList = function () {
  let isEdit = $('input[name=quote_edit]').val(),
    ...
    queryParam = '&campus__in=' + campusId + '&show_quote_tool__eq=1&program_type__in=' + programName + '&sort=program_name',
...
const getProgramsPriceList = function () {
  ...
  queryParam1 = '&campus_location__in=' + campusId + '&category__eq=' + programCategory + '&programs_available__in=' + programId + '&enable__eq=1&sort=price_current',
  ...
  if (listArray.length > 0) {
    ...
    for (let i = 0; i < listArray.length; i++) {
      ...
      if (listArray[i].price_additional) {
        priceAdditionalConversion = round(xConversion(listArray[i].price_additional), 1).toFixed(2);
      }
      if (listArray[i].price_additional_2) {
        priceAdditionalConversion2 = round(xConversion(listArray[i].price_additional_2), 1).toFixed(2);
      }
      price = currencyFormatter(priceConversion);
...
const updateProgramSelectionTotalPrice = function () {
  ...
  quoteJson.quote_array[objPosition].program.material_fee = parseFloat(programMaterialFee);
  ...
  quoteJson.quote_array[objPosition].program.program_total_price = parseFloat(programTotalPrice);
  quoteJson.quote_array[objPosition].program.program_total_standard_price = parseFloat(programStandardPrice);
...
const getStartDates = function () {
  ...
  if (programCategory.indexOf('Camps') !== -1) {
    queryParam = '&program__in=' + programId + '&campus__in=' + campusId + '&start_dates__gt=' + timestamp + '&fixed_week_duration__in=' + programDuration + '&sort=start_dates';
  } else {
...
const setTotalPrice = function () {
  ...
  totalPrice = schedulePriceSelected + materialFeeSelected + additionalFeeSelected + registrationFeeSelected + accommodationTotalSelected + airportTransferFeeSelected + accommodationPlacementFeeSelected + accommodationPlacementFeeSelected + totalAddPrice;
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        addChildVal = parseFloat(quoteObj.quote_array[i].accommodation.family_additional_child);
        addAdultVal = parseFloat(quoteObj.quote_array[i].accommodation.family_additional_adult);
        addChildPrice = parseFloat(quoteObj.quote_array[i].accommodation.family_additional_child_total_price);
        addAdultPrice = parseFloat(quoteObj.quote_array[i].accommodation.family_additional_adult_total_price);
