...
const getProgramsList = function () {
  let isEdit = $('input[name=quote_edit]').val(),
    ...
    queryParam = '&campus__in=' + campusId + '&show_quote_tool__eq=1&category__eq=Programs&sort=program_name',
    ...
    $.get(api_url).done(function (data) {
      ...
      if (dataObject.length > 0) {
        for (let i = 0; i < dataObject.length; i++) {
          ...
          programMinAge = dataObject[i].values.minimum_age ? dataObject[i].values['minimum_age'][0].name : campusMinAge;
          programMaxAge = dataObject[i].values.maximum_age ? dataObject[i].values['maximum_age'][0].name : campusMaxAge;
...
const getProgramsPriceListLowest = function () {
  let isEdit = $('input[name=quote_edit]').val(),
    ...
    queryParam = '&campus_location__in=' + campusId + '&programs_available__in=' + programName + '&is_program_price__eq=1&enable__eq=1&sort=-price_current',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
    programPriceArray = [];
  $('#online-msg').remove();
...
const getStartDates = function () {
  let isEdit = $('input[name=quote_edit]').val(),
    ...
    queryParam = '&program__in=' + programName + '&campus__in=' + campusId + '&start_dates__gt=' + timestamp + '&sort=start_dates',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam;
  api_url = encodeURI(api_url);
...
const updateStartDateSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    monthValue = "0" + $('.selectBox .selectBox__value[data-option=month]').attr('data-selected'),
    dayValue = "0" + $('.selectBox .selectBox__value[data-option=day]').attr('data-selected'),
...
const getOtherProgramFees = function () {
  let campus = $('input[name=campus_selection]').val(),
    ...
    programName = $('input[name=program_selection]').val(),
    tableId = schoolPriceListTable,
    queryParam = '&campus_location__in=' + campusId + '&category__eq=Other Program Fees&is_program_price__eq=0&programs_available__in=' + programName + '&enable__eq=1&sort=-price_current',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
    feeArray = [];
  api_url = encodeURI(api_url);
...
const setTotalPrice = function () {
  ...
  let quoteObj = JSON.parse(localStorage.getItem("quote-obj")),
    ...
    additionalFeeSelected = parseFloat(quoteObj.quote_array[editNum].program.additional_fee),
  ...
  promoAutoDiscountTotal = 0,
    totalPrice = schedulePriceSelected + materialFeeSelected + additionalFeeSelected + registrationFeeSelected + accommodationTotalSelected + airportTransferFeeSelected + accommodationPlacementFeeSelected + accommodationCleaningFeeSelected;
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        priceAdditionalVal = parseFloat(quoteObj.quote_array[i].program.additional_fee);
        ...
        prePriceQuoteTotalVal = priceProgramVal + priceRegistrationVal + priceAdditionalVal + priceMaterialVal + priceAccommodationVal + priceAirportVal + priceAccommodationPlacementFeeVal + priceAccommodationCleaningFeeVal;
        priceQuoteTotalVal = parseFloat(quoteObj.quote_array[i].quote_total_price);
        quoteShowCurrency = quoteObj.quote_array[i].show_currency;
        ...
        if (priceProgramVal < priceStandardVal) {
          ...
          discountHtml = "<span class='discount-txt strike discount-program-price'>" + currencyFormatter(priceStandardVal, quoteShowCurrency) + "</span>";
          discountTotalHtml = "<div class='total-discount-line'><div class='item-description total-title discount-title-price'>Discount Total</div><div class='item-price'><span class='discount-txt discount-total-price'>" + currencyFormatter(-discountVal, quoteShowCurrency) + "</span></div></div>";
        }
        ...
        minorAccommodationText = 'If you are under 18 at the time of arrival and will not be accompanied by a parent or guardian, an additional Unaccompanied Minor Homestay Booking Fee of $280; and, if applicable, an additional Unaccompanied Minor Airport Pickup Fee of $150 will apply.'

        isMinorAccomm = quoteObj.quote_array[i].accommodation.is_minor ? true : false;

        minorAccommodationHtml = isMinorAccomm ? "<div class='center footnote'>" + minorAccommodationText + "</div>" : '';
        ...
        totalPriceHtml = "<div class='line-item'>" + discountTotalHtml + "<div><div class='item-description total-title bold'>TOTAL</div><div class='item-price bold total-price'>" + currencyFormatter(priceQuoteTotalVal, quoteShowCurrency) + "</div></div></div>";
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        campusCurrency = quoteObj.quote_array[i].campus_currency;
        reset = currency ? currency != quoteObj.quote_array[i].show_currency ? true : false : quoteObj.quote_array[i].show_currency == campusCurrency ? false : true;

        if (reset) {
          if (!currency) {
            fromCurrency = quoteObj.country_origin_currency;
            toCurrency = campusCurrency;
            convert = true;
            quoteObj.quote_array[i].show_currency = toCurrency;
          } else {
            toCurrency = currency;
            fromCurrency = campusCurrency;
            quoteObj.quote_array[i].show_currency = toCurrency;
            convert = false;
          }
