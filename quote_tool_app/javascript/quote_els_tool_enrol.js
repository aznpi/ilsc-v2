...
$(".campus-selection-container .dropdown-menu li.campus-name").on(
  ...
  function () {
    let getValue = $(this).text(),
      ...
      getMinAge = $(this).attr("data-campus-min-age")
        ? $(this).attr("data-campus-min-age")
        : 0,
      getMaxAge = $(this).attr("data-campus-max-age")
        ? $(this).attr("data-campus-max-age")
        : 99;
...
const getProgramsList = function () {
  let isEdit = $("input[name=quote_edit]").val(),
    ...
    queryParam =
      "&campus__in=" +
      campusId +
      "&show_quote_tool__eq=1&category__eq=Programs&sort=program_name",
      ...
      .done(function (data) {
        ...
        if (dataObject.length > 0) {
          for (let i = 0; i < dataObject.length; i++) {
            ...
            programMinAge = dataObject[i].values.minimum_age
              ? dataObject[i].values["minimum_age"][0].name
              : campusMinAge;
            programMaxAge = dataObject[i].values.maximum_age
              ? dataObject[i].values["maximum_age"][0].name
              : campusMaxAge;
...
const getProgramsPriceListLowest = function () {
      ...
      success: function (data) {
        ...
        for (let i = 0; i < dataObject.length; i++) {
          ...
          programPriceArray.push({
            ...
            item_name: dataObject[i].values["item_name"].name,
  ...
  if (campusName == "Online") {
    onlineHtml =
      "<div id='online-msg' class='center disclaimer'><p>See our <a href='https://www.ilsc.com/hubfs/pdf/others/online-english-program-schedule-time-zones.pdf' target='_blank'>Online Schedules Time Zones</a> to find out which schedule works best for you. </p></div>";
    $(".program-price-list-selection-container .step-content-container").after(
      onlineHtml
    );
  }
};
...
const dateDropValidation = function (className) {
  ...
  if (currentVal != optionVal) {
    if (currentOption == "day") {
      if (
        $(".start-date-selection-container .selectDuration").hasClass("active")
      ) {
        $(".start-date-selection-container .selectDuration").removeClass(
          "show"
        );
        $(".start-date-selection-container .selectDuration .selectBox__value")
          .removeClass("active")
          .addClass("disabled")
          .attr("data-selected", "")
          .text("Duration");
      } else {
        $(".start-date-selection-container .selectDuration")
          .addClass("active")
...
$(document).on(
  ...
  function (e) {
    ...
    let yearValue = $(
      ...
      dayValue = $(
        ".start-date-selection-container .selectBox .selectBox__value[data-option=day]"
      ).attr("data-selected"),
      durationValue = $(
        ".start-date-selection-container .selectBox .selectBox__value[data-option=duration]"
      ).attr("data-selected");
...
const printStartDateSelection = function () {
  ...
  let groupId = $("input[name=program_price_selection]:checked").val(),
    programName = $("input[name=program_price_selection]:checked").attr(
      "data-program-name"
    ),
    itemName = $("input[name=program_price_selection]:checked").attr(
      "data-item-name"
    ),
...
const printDurationSelection = function (weekArray) {
  let className = $(
      ".start-date-selection-container .date-container #duration-select .selectDuration .dropdown-menu"
    ),
    htmlOption = "",
    minChar = ">",
    maxChar = "<",
...
const getStartDates = function () {
  ...
  if (getDates) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (let i = 0; i < dataObject.length; i++) {
          ...
          epochTime = dataObject[i].values.start_dates;
          ...
          if (dataObject[i].values.fixed_week_duration) {
            for (
              ...
              weekArray.push({
                weeks: dataObject[i].values["fixed_week_duration"][x].name,
              });
            }
          }
    ...
    if (dateArray.length > 0) {
      ...
      $(".start-date-selection-container .campus-age-disclaimer").html(
        '<p class="text-center">No start dates available. Possible reason: The minimum age requirement at the ' +
          campus +
          " campus at the start of program is: " +
          minAge +
          "</p>"
      );
    }
...
const resetPrintDates = function () {
  let yearVal = $(
    ...
    dayVal = $(
      ".date-container #day-select .selectBox .selectBox_value.active"
    ).attr("data-selected"),
    weekVal = $(
      ".date-container #duration-select .selectBox .selectBox_value.active"
    ).attr("data-selected"),
  ...
  if (timeVal == "week") {
      ...
      if (year == dataYear) {
        for (let month in startDateArray[year][0].month) {
          if (month == dataMonth) {
            for (
              let x = 0;
              x < startDateArray[year][0].month[month].length;
              x++
            ) {
              if (dataItem == startDateArray[year][0].month[month][x].day) {
                for (
                  let i = 0;
                  i <
                  startDateArray[year][0].month[month][x].week_available.length;
                  i++
                ) {
                  weekValArray.push({
                    week: startDateArray[year][0].month[month][x]
                      .week_available[i].weeks,
                  });
                }
              }
            }
    ...
    for (let year in startDateArray) {
      if (year == dataYear) {
        for (let month in startDateArray[year][0].month) {
          if (month == dataItem) {
            startDateArray[year][0].month[month].sort((a, b) => a.day - b.day);
            for (let day in startDateArray[year][0].month[month]) {
              dayVal = startDateArray[year][0].month[month][day].day;
              dayHtml =
                '<a href="#" class="dropdown-item" data-parent="day" data-item="' +
                dayVal +
                '" data-year="' +
                year +
                '" data-month="' +
                ...
                dayVal +
                "</a>";
              $(".date-container #day-select .selectBox .dropdown-menu").append(
                dayHtml
              );
            }
          }
      ...
      if (year == dataItem) {
        for (let month in startDateArray[year][0].month) {
          ...
          monthHtml =
            ...
            m +
            "</a>";
          $(".date-container #month-select .selectBox .dropdown-menu").append(
            monthHtml
          );
        }
      }
    ...
    $(".selectBox,.selectBox__value", ".date-container #year-select")
      .removeClass("disabled")
      .addClass("active");
    $(".date-container #month-select .selectBox .dropdown-menu").empty();
    $(".date-container #day-select .selectBox .dropdown-menu").empty();
...
const updateStartDateSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    monthValue =
      "0" +
      $(".selectBox .selectBox__value[data-option=month]").attr(
        "data-selected"
      ),
    dayValue =
      "0" +
      $(".selectBox .selectBox__value[data-option=day]").attr("data-selected"),
    durationValue = $(
    ...
    itemName = $("input[name=program_start_date_selection]").attr(
      "data-item-name"
    ),
    priceFormat = $("input[name=program_start_date_selection]").attr(
      "data-price-format"
    );

  (monthValue = monthValue.slice(-2)), (dayValue = dayValue.slice(-2));
  ...
  for (let i = 0; i < setFeeArray.length; i++) {
    if (setFeeArray[i].itemName.includes("Fee")) {
      if (setFeeArray[i].itemName.includes("Registration")) {
        ...
        feeVal =
          setFeeArray[i].priceType == "Per week"
            ? setFeeArray[i].feePrice * durationNum
            : setFeeArray[i].feePrice;
        ...
        quoteJson.quote_array[objPosition].program.additional_fee_description =
          setFeeArray[i].feeDescription;
        quoteJson.quote_array[objPosition].program.additional_item_name =
          setFeeArray[i].itemName;
      }
    } else {
      addFeeVal =
        setFeeArray[i].priceType == "Per week"
    ...
    success: function (data) {
      ...
      for (let i = 0; i < dataObject.length; i++) {
        ...
        feeArray.push({
          ...
          itemName: dataObject[i].values["item_name"].name,
        });
      }
...
const setTotalPrice = function () {
  ...
  (promoAutoDiscountTotal = 0),
    (totalPrice =
      ...
      accommodationCleaningFeeSelected);
    ...
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        scheduleProgramHtml =
          ...
          quoteObj.quote_array[i].program.option_schedule +
          "<br>(" +
          quoteObj.quote_array[i].program.duration +
          " Weeks)</span></div><div class='item-notes'>Start: " +
        ...
        if (priceAdditionalVal > 0) {
          additionalHtml =
            ...
            quoteObj.quote_array[i].program.additional_item_name +
            " - " +
        ...
        if (priceMaterialVal > 0) {
          materialHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Material Fee</span></div><div class='item-notes'>per week</div></div><div class='item-price'><span class='bold material-fee'>" +
        ...
        if (priceAccommodationVal > 0) {
          ...
          placementFeePriceHtml =
            priceAccommodationPlacementFeeVal > 0
              ? "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Housing Booking Fee</span></div></div><div class='item-price'><span class='bold placement-fee'>" +
                currencyFormatter(
                  priceAccommodationPlacementFeeVal
                ) +
                "</span></div></div></div>"
        ...
        if (priceAirportVal > 0) {
          airportPriceHtml =
            ...
            "</span></div></div></div>";
          airportHtml =
            "<div class='quote-airport quote-box'>" +
            airportPriceHtml +
            "</div>";
        }

        insuranceText =
          "All ELS students must have medical insurance coverage while in the U.S.A. If you cannot provide evidence of sufficient coverage, in English, you must purchase the ELS Student Health Plan. The Student Health Plan is offered to ELS students at a non-refundable fee of $50 per week. You will receive additional information about the Student Health Plan when you arrive at the center.";
        insuranceNoteHtml =
          "<div class='center footnote'>" + insuranceText + "</div>";

        minorAccommodationText =
          "If you are under 18 at the time of arrival and will not be accompanied by a parent or guardian, an additional Unaccompanied Minor Homestay Booking Fee of $280; and, if applicable, an additional Unaccompanied Minor Airport Pickup Fee of $150 will apply.";

        isMinorAccomm = quoteObj.quote_array[i].accommodation.is_minor
          ? true
          : false;

        minorAccommodationHtml = isMinorAccomm
          ? "<div class='center footnote'>" + minorAccommodationText + "</div>"
          : "";
        ...
        totalPriceHtml =
          ...
          "</div></div></div>";
        quoteHTML =
          "<div class='col col-md-6 col-quote-" +
          quoteNum +
          ...
          quoteNum +
          "' tabindex='0'>Remove <span class='icon-remove'>&#8999;</span></button></div><div class='btn-container enroll-btn-container'><button type='button' class='btn btn-orange-outline bold quote-enroll center' data-edit-num='" +
          quoteNum +
          "' data-target='enroll-form-quote' tabindex='0' data-bs-toggle='tooltip' data-bs-placement='right' data-bs-html='true' title='Choose this quote and <strong><em>Enroll</em></strong> now!'>Enroll</button></div>";
