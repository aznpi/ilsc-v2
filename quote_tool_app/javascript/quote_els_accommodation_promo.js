...
const returnHousingFee = function (data) {

  let quoteNum = $('input[name=quote_number]').val(),
    editNum = quoteNum - 1;
  quoteObject = localStorage.getItem(quoteJsonName),
    quoteJson = JSON.parse(quoteObject),
    dataObject = data.results,
    weekStay = parseInt(quoteJson.quote_array[editNum].accommodation.length_of_stay),
    checkInDate = quoteJson.quote_array[editNum].accommodation.check_in_date,
    checkInDateEpoch = parseInt(new Date(checkInDate).getTime()),
    dobEpoch = parseInt($('input[name=dob]').val()) * 1000,
    ageAtCheckin = returnAge(dobEpoch, checkInDateEpoch),
    ageConfirm = true,
    parentConfirm = true;

  console.log('dob: ' + dobEpoch);
  console.log('check in Date: ' + checkInDateEpoch);

  console.log(ageAtCheckin);


  if (dataObject.length > 0) {

    rangeVal = dataObject[0].values.weeks ? dataObject[0].values.weeks.name : false;
    isUnderAge = dataObject[0].values.under_age == 1 ? true : false;
...
const getAccommodationPlacementFee = function (year) {
  let campus = $('input[name=campus_selection]').val(),
    ...
    tableId = accommodationPriceListTable,
    queryParam = '&campus_location__in=' + campusId + '&category__eq=Other Accommodation Fees&item_description__contains=Housing Booking Fee&enable__eq=1&sort=price_current',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
    quoteNum = $('input[name=quote_number]').val(),
    quoteObject = localStorage.getItem(quoteJsonName),
    quoteJson = JSON.parse(quoteObject),
...
const printAccomodationForm = function (enable) {
  ...
  $(".accommodation-price-selection-container").click('.ui-datepicker-calendar a.ui-state-default,a.dropdown-item,input[name=accommodation_price_selection],input[name=airport_selection]', function () {
    ...
    let campusCountrySelected = $('input[name=campus_country_selection]').val(),
      ...
      additionalNotePlain = accommmodationName.includes('Residence') ? $('input[name=accommodation_price_selection]:checked').attr('data-additional-note-plain') : campusCountrySelected === 'Canada' ? homestayNote : '',
