...
const getProgramsList = function () {
  let isEdit = $('input[name=quote_edit]').val(),
    ...
    queryParam = '&campus__in=' + campusId + '&show_quote_tool__eq=1&program_type__ne=' + omitProgram + '&category__eq=Programs&sort=program_name',
    ...
    $.get(api_url).done(function (data) {
      ...
      if (dataObject.length > 0) {
        for (let i = 0; i < dataObject.length; i++) {
          ...
          radioHtml = "<div class='program-option'><input type='radio' autocomplete='off' class='program-radio' name='program_radio_selection' id='" + programId + "' data-id='" + id + "' data-description='" + programDescription + "' value='" + programName + "' " + selectEdit + "/><label for='" + programId + "'>" + programName + "</label></div>";
...
const printDurationSelection = function (weekArray) {
  let className = $('.start-date-selection-container .date-container #duration-select .selectDuration .dropdown-menu'),
    parentEl = $('.start-date-selection-container .date-container #duration-select'),
    countrySelected = $('input[name=country_origin_selection]').val(),
    programSelected = $('input[name=program_selection]').val(),
    htmlOption = "",
    minChar = '>',
    maxChar = '<',
    ...
    durationNote = countrySelected == 'Ireland' && programSelected !== 'Study and Work' ? '<div id="duration-note"><sup>*</sup>Non EU/EEA students on a Tourist Visa / Visa Waiver may study a maximum of 12 weeks</div>' : '';
...
const printQuotesHTML = function () {
  let destinationCurrency = $('input[name=destination_currency_selection]').val();
  ...
  $('.x-convert-container .btn[data-destination-currency]').removeClass('active').removeClass('btn-orange-outline').addClass('btn-black-outline').text(destinationCurrency);
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        quoteHTML = "<div class='col col-md-6 col-quote-" + quoteNum + "'><div class='quote-title'>Quote " + quoteNum + "</div><div class='quote-select-container'><div class='quote-items'><div class='quote-location'>" + schoolHtml + campusHtml + "</div><div class='quote-program-schedule quote-box'>" + scheduleProgramHtml + registrationHtml + additionalHtml + materialHtml + "</div>" + accommodationHtml + promoAutoHtml + "<div class='promo-code-box'>" + promoCodeHtml + "</div>" + printExceptionHtml + "<div class='quote-box total-price-container'>" + totalPriceHtml + "</div>" + accommodationNoteHtml + priceNoteHtml + "</div><div class='promo-container'><label for='input-promo-code-" + quoteNum + "'>Enter Promo Code</label><div><input type='text' id='input-promo-code-" + quoteNum + "' class='input-promo-code' name='input-promo-code-" + quoteNum + "'><button data-quote='" + quoteNum + "' class='btn-promo-code btn-orange'>APPLY</button></div><div id='promo-feedback-" + quoteNum + "' class='promo-feedback'></div></div><div><div class='btn-container'><button type='button' class='btn btn-orange-outline bold quote-edit' data-edit-num='" + quoteNum + "' tabindex='0'>Edit</button><button type='button' class='btn btn-orange-outline bold quote-remove' data-remove-num='" + quoteNum + "' tabindex='0'>Remove <span class='icon-remove'>&#8999;</span></button></div></div>";
