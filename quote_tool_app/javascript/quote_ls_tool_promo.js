...
const updateLocalJson = function (dataStep, editNum) {
  if (dataStep == "campus-select") {
    ...
    getProgramPriceDuration();
    setTimeout(updateStartDateSelection(editNum), 500);
    getPromo("Automatic");
...
$(".campus-selection-container .dropdown-menu li.campus-name").on(
  ...
  function () {
    ...
    setCheckInCalendar(getCountryValue);
  }
);
...
const getProgramsList = function () {
  let isEdit = $("input[name=quote_edit]").val(),
    ...
    omitProgram = "Junior Programs",
    tableId = schoolProgramListTable,
    queryParam =
      "&campus__in=" +
      campusId +
      "&show_quote_tool__eq=1&program_type__ne=" +
      omitProgram +
      "&category__eq=Programs&sort=program_name",
    ...
    $.ajax({
      ...
      success: function (dataP) {
        let dataPriceObject = dataP.results,
          priceLocal = JSON.parse(localStorage.getItem("program-price")) || [];
...
const printDurationSelection = function (weekArray) {
  let className = $(
    ...
    parentEl = $(
      ".start-date-selection-container .date-container #duration-select"
    ),
    countrySelected = $("input[name=campus_country_selection]").val(),
    programSelected = $("input[name=program_selection]").val(),
    htmlOption = "",
    minChar = ">",
    maxChar = "<",
...
const updateStartDateSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    dayValue = $(".selectBox .selectBox__value[data-option=day]").attr(
      "data-selected"
    ),
    durationValue = $(
  ...
  for (var i = 0; i < quoteJson.quote_array.length; i++) {
    if (i === objPosition) {
      ...
      quoteJson.quote_array[i].program.duration_note = durationNote;
...
const setOtherProgramFees = function (feeArray) {
  ...
  for (let i = 0; i < setFeeArray.length; i++) {
    ...
    if (setFeeArray[i].feeType == "Exam Fees") {
      let examFeeVal = setFeeArray[i].feePrice;
      quoteJson.quote_array[objPosition].program.exam_fee = round(
        xConversion(examFeeVal),
        1
      ).toFixed(2);
      quoteJson.quote_array[objPosition].program.exam_fee_description =
        setFeeArray[i].feeDescription;
    }
  }
  localStorage.setItem(quoteJsonName, JSON.stringify(quoteJson));
...
const setTotalPrice = function () {
  ...
  let quoteObj = JSON.parse(localStorage.getItem("quote-obj")),
    ...
    examFeeSelected = parseFloat(
      quoteObj.quote_array[editNum].program.exam_fee
    ),
  ...
  if (quoteObj.quote_array.length > 0) {
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        priceExamVal = parseFloat(quoteObj.quote_array[i].program.exam_fee);
        ...
        if (priceExamVal > 0) {
          examHtml =
            "<div class='line-item'><div><div class='item-description'><div class='item-label'><span class='label-title bold'>Exam Fee</span></div><div class='item-notes'>" +
            quoteObj.quote_array[i].program.exam_fee_description +
            "</div></div><div class='item-price'><span class='bold exam-fee'>" +
            currencyFormatter(priceExamVal) +
            "</span></div></div></div>";
        }
    ...
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        if (fromCurrency != toCurrency) {
          ...
          priceExamVal = convertCalcFloat(
            quoteObj.quote_array[i].program.exam_fee,
            convert
          );
          priceExamVal = round(priceExamVal, 1).toFixed(2);
          quoteObj.quote_array[i].program.exam_fee = priceExamVal;
          ...
          $(".col-quote-" + quoteNum + " span.exam-fee").html(
            currencyModFormatter(priceExamVal, toCurrency)
          );
...
const sendEmailQuoteForm = function () {
  ...
  for (let i = 0; i < quoteObj.quote_array.length; i++) {
    ...
    examFee = parseFloat(quoteObj.quote_array[i].program.exam_fee);
    examFee = examFee.toFixed(2);
    ...
    if (quoteObj.quote_array[i].quote_enabled) {
      formFieldsArray.push(
        ...
        {
          objectTypeId: "0-1",
          name: "quote_" + quoteNum + "_program_schedule_duration_note",
          value: quoteObj.quote_array[i].program.duration_note,
        },
        {
          objectTypeId: "0-1",
        ...
        {
          objectTypeId: "0-1",
          name: "quote_" + quoteNum + "_exam_fee",
          value: currencyModFormatter(examFee, showCurrency),
        },
        {
          objectTypeId: "0-1",
          name: "quote_" + quoteNum + "_exam_fee_description",
          value: quoteObj.quote_array[i].program.exam_fee_description,
        },
