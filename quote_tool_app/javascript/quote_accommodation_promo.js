...
const redirectAccommodationNoAge = function () {
  ...
  if (checkedVal == 'yes-accomm-option') {
    ...
    setTimeout(setTotalPrice, 300);
    setTimeout(printQuotesHTML, 500);
    quoteSlide = $('.quote-step-slide .slick-slide').index($('#quote-summary'));
    $('#quote-price-container').attr('data-complete', true);
    $('.quote-step-slide').slick('slickGoTo', quoteSlide);
  }
}
...
const getHomeStayPriceList = function () {
  let campus = $('input[name=campus_selection]').val(),
    ...
    tableNotesId = accommadationNotesTable,
    ...
    queryParamNotes = '&campus__in=' + campusId + '&accommodation_type__in=Homestay',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
    api_url_notes = apiUrl + tableNotesId + '/rows?portalId=' + portalId + queryParamNotes;
  api_url = encodeURI(api_url);
  api_url_notes = encodeURI(api_url_notes);
  let homestayArray = [],
    notesArray = [];
    ...
    success: function (data) {
      ...
      if (dataObject.length > 0) {
        for (let i = 0; i < dataObject.length; i++) {
          notesArray.push({
            notes_id: dataObject[i].id,
            accommodation_note_name: dataObject[i].values['accommodation_note_name'].name,
            accommodation_additional_note: dataObject[i].values['accommodation_additional_note']
          })
        }
      }
    }
    ...
    success: function (data) {
      ...
      if (dataObject.length > 0) {
        for (let i = 0; i < dataObject.length; i++) {
          ...
          accommodationNoteId = dataObject[i].values['accommodation_price_notes'] ? dataObject[i].values['accommodation_price_notes'][0].id : '';
          ...
          homestayArray.push({
            ...
            accommodation_note_id: accommodationNoteId
          });
        }

        let mergeHomestayArray = homestayArray.map(item => { return { ...item, accommodation_notes: notesArray.filter(item2 => item2.notes_id === item.accommodation_note_id) } })

        let programAccommodationGroupedByGroupId = groupBy(mergeHomestayArray, 'group_id');
          ...
          for (let x = 0; x < values[i].length; x++) {
            if (x == 0) {
              lowestPriceArray.push({
                ...
                'accommodation_notes': values[i][x].accommodation_notes
...
const printHomestayPriceList = function (priceArray) {
  ...
  for (let i = 0; i < priceArray.length; i++) {
    ...
    accommodationNote = priceArray[i].accommodation_notes.length > 0 ? priceArray[i].accommodation_notes[0].accommodation_additional_note.replace(/<[^>]*>/g, '') : '';
  ...
  optionListHtml = optionHtml + homestayNoteHTML;

  $(".homestay-select-container .option-container").html(optionListHtml);
...
const getResidencePriceList = function () {
  let campus = $('input[name=campus_selection]').val(),
    ...
    queryParamNotes = '&campus__in=' + campusId + '&accommodation_type__in=Residence',
  ...
  $.when(
    ...
    if (dataObject1.length > 0) {
      ...
      resArray = dataObject1.map(std => {

        accNoteId = std.values['accommodation_price_notes'] ? std.values['accommodation_price_notes'][0].id : '';
        return {
          accommodation_id: std.id,
          accommodation_name: std.values.item_name,
          accommodation_note_id: accNoteId,
          item_name: std.values.item_name,
      ...
      residenceMergeArray = resArray.map(item => { return { ...item, accommodation_notes: notesArray.filter(item2 => item2.notes_id === item.accommodation_note_id) } })
      ...
      for (let i = 0; i < residenceMergeArray.length; i++) {
        ...
        additionalNote = residenceMergeArray[i].accommodation_notes.length > 0 ? residenceMergeArray[i].accommodation_notes[0].accommodation_additional_note : '';

        residenceArray.push({
          accommodation_id: residenceMergeArray[i].accommodation_id,
          accommodation_name: residenceMergeArray[i].accommodation_name,
          item_name: residenceMergeArray[i].foreign_array[0].values.name,
          ...
          accommodation_note: additionalNote,
          accommodation_note_plain: additionalNote.replace(/<[^>]*>/g, '')
        });
      }
...
const getAccommodationPlacementFee = function (year) {
  let campus = $('input[name=campus_selection]').val(),
    ...
    tableId = accommodationPriceListTable,
    queryParam = '&campus_location__in=' + campusId + '&category__eq=Other Accommodation Fees&price_description__contains=Accommodation Administration Fee&enable__eq=1&sort=price_current',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
...
const getAccommodationCleaningFee = function (year) {
  let campusId = $('input[name=campus_id_selection]').val(),
    accomPartnerId = $('input[name=accommodation_group_id_selection]').val(),
    tableId = accommodationPriceListTable,
    queryParam = '&campus_location__in=' + campusId + '&group_code__in=' + accomPartnerId + '&category__eq=Other Accommodation Fees&item_name__contains=Residence Additional Fees&price_description__contains=Cleaning Fee&enable__eq=1&sort=price_current',
    api_url = apiUrl + tableId + '/rows?portalId=' + portalId + queryParam,
    quoteNum = $('input[name=quote_number]').val(),
    quoteObject = localStorage.getItem(quoteJsonName),
    quoteJson = JSON.parse(quoteObject),
...
const printAccomodationForm = function () {
  ...
  $(".accommodation-price-selection-container").click('.ui-datepicker-calendar a.ui-state-default,a.dropdown-item,input[name=accommodation_price_selection],input[name=airport_selection]', function () {
    ...
    let campusCountrySelected = $('input[name=campus_country_selection]').val(),
      accommodationId = $('input[name=accommodation_price_selection]:checked').val(),
      ...
      accommmodationName = $('input[name=accommodation_price_selection]:checked').attr('data-program-name') ? $('input[name=accommodation_price_selection]:checked').attr('data-program-name').toString() : '',
      ...
      additionalNotePlain = accommmodationName.includes('Residence') ? $('input[name=accommodation_price_selection]:checked').attr('data-additional-note-plain') : campusCountrySelected === 'Canada' ? additionalNote : '',
