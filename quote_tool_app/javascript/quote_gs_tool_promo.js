...
const updateCampusSelection = function (editNum) {
  let objPosition = editNum - 1,
    ...
    ).attr("data-enable-default-currency"),
    campusSelected = $(
      ".campus-selection-container .campus-select[data-selected=true]"
    ...
    currencyShow =
      isDefaultCurrency === "true" ? campusCurrencySelected : initShowCurrency; 
  ...
  for (var i = 0; i < quoteJson.quote_array.length; i++) {
    if (i === objPosition) {
      ...
      quoteJson.quote_array[i].show_currency = currencyShow;
...
$(".campus-selection-container .dropdown-menu li.campus-name").on(
  ...
  function () {
    ...
    $(this)
      ...
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-selected-campus", getCountryValue);
    $(this)
      .parent()
      .siblings("button.campus-select")
      .attr("data-selected", true)
      .attr("data-program-area", getValue);
    $(this)
      .parent()
      .siblings("button.campus-select")
      .attr("data-selected", true)
...
$("#program-list-container,#program-coop-list-container").on(
  "click",
  "input[type=radio]",
  function () {
...
const getProgramsList = function () {
  let isEdit = $("input[name=quote_edit]").val(),
    ...
    queryParam =
      "&program_areas__in=" +
      programName +
      "&show_quote_tool__eq=1&country__in=" +
      countryName +
      "&is_parent_program__eq=1&is_co_op__neq=1&sort=program_name",
    queryParamCoop =
      "&program_areas__in=" +
      programName +
      "&show_quote_tool__eq=1&country__in=" +
      countryName +
      "&is_co_op__eq=1&sort=program_name",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam,
    api_url_coop =
      apiUrl + tableId + "/rows?portalId=" + portalId + queryParamCoop;
  ...
  if ($("#program-list-container").children().length == 0) {
    ...
    $.get(api_url)
      .done(function (data) {
        let dataObject = data.results.filter(
            (elem, index) =>
              data.results.findIndex(
                (obj) =>
                  obj.values["parent_program_code"][0].id ===
                  elem.values["parent_program_code"][0].id
              ) === index
          ),
          radioHtml = "";
        ...
        if (dataObject.length > 0) {
          for (let i = 0; i < dataObject.length; i++) {
            ...
            if (isEdit === "true") {
              if (programName == isEditProgramName) {
                selectEdit = "checked";
                $(
                  '#program-list-container .program-option input[value="' +
                    programName +
                    '"]'
                ).click();
              }
            ...
            radioHtml =
              radioHtml +
              "<div class='program-option'><input type='radio' autocomplete='off' class='program-radio' name='program_radio_selection' data-id='" +
              programId +
              "' id='" +
              programId +
              "' data-is-coop='false' data-description='" +
              programDescription +
              "' value='" +
              programName +
              "' " +
          ...
          $("#program-list-container").html(
            '<h3 class="title text-center">ACADEMIC PROGRAMS</h3>' + radioHtml
          );
        } else {
          radioHtml =
            "<div class='text-center'><h3>ACADEMIC PROGRAMS</h3><h4>No Programs Available at this time.</h4></div>";
    ...
    $.get(api_url_coop)
      .done(function (data) {
        ...
        if (dataObject.length > 0) {
          for (let i = 0; i < dataObject.length; i++) {
            ...
            if (isEdit === "true") {
              if (programName == isEditProgramName) {
                selectEdit = "checked";
                $(
                  '#program-coop-list-container .program-option input[value="' +
                    programName +
                    '"]'
                ).click();
              }
            ...
            radioHtml =
              ...
              " id='" +
              programId +
              "-COOP' data-is-coop='true' data-description='" +
              programDescription +
              "' value='" +
              programName +
              "' " +
          ...
          $("#program-coop-list-container")
            .html(
              '<h3 class="title text-center">CO-OP PROGRAMS</h3>' + radioHtml
            )
            .show();
        } else {
      ...
      .fail(function () {
        radioHtml =
          "<div class='text-center'><h3>CO-OP PROGRAMS</h3><h4>No Programs Available at this time.</h4></div>";
...
const updateProgramSelection = function (editNum) {
  ...
  for (var i = 0; i < quoteJson.quote_array.length; i++) {
    if (i === objPosition) {
      quoteJson.quote_array[i].program_school_name =
        programSchoolSelected + coopStr;
...
const printProgramAreaTitle = function () {
  ...
  $(
    ".program-list-selection-container.step-container.program-container .program-title.step-title h3"
  ).text(programTitle);
};
const printProgramHeaderTitle = function () {
  ...
  $(
    ".program-price-list-selection-container.step-container.program-container .program-title.step-title h3, .start-date-selection-container.step-container.program-container .program-title.step-title h3"
  ).text(programTitle);
  $(
...
const getProgramsPriceList = function () {
  ...
  (queryParam1 =
    "&country__in=" +
    campusCountryName +
    "&parent_program_code__in=" +
    programId +
    "&enable__eq=1&is_co_op__eq=" +
    isCoop +
    "&sort=price_current"),
    ...
    (queryParam2 =
      ...
      isCoop),
    (api_url_2 =
      apiUrl + marketTableId + "/rows?portalId=" + portalId + queryParam2),
    (queryParam3 = "&schools__in=81771084433"),
    (api_url_3 =
      apiUrl + campusTableId + "/rows?portalId=" + portalId + queryParam3),
    (programPriceArray = []),
    (objects = []);
      ...
      .then(function () {
        ...
        if (dataObject2.length > 0) {
          programPriceArray = dataObject1.map((std) => ({
            ...
            market_price: dataObject2.filter(
              (mark) => mark.values["price_list_code"][0].id === std.id
            ),
          }));
          ...
          for (let i = 0; i < programPriceArray.length; i++) {
            for (let x = 0; x < programPriceArray[i].campus.length; x++) {
              ...
              campusArr = dataObject3.filter(
                (c) => c.id === programPriceArray[i].campusId[x].id
              );
        ...
        let programPriceGroupedByGroupCampus = groupBy(
            campusPriceArray,
            "campusId"
          ),
...
const printStandardPriceProgramList = function (listArray) {
  ...
  if (listArray.length > 0) {
    ...
    for (let i = 0; i < listArray.length; i++) {
      ...
      for (let x = 0; x < listArray[i].length; x++) {
        ...
        materialFee = round(
          xConversion(listArray[i][x].material_fee),
          1
        ).toFixed(2);
        ...
        if (marketPriceConversion < priceConversion) {
          radioHtml =
            ...
            "</div><div class='price-container text-right float-right'><span class='discount-txt strike'>" +
            price +
  ...
  $("#program-price-list-container >div.program-option").click(function () {
    let campusSelected = $("input[name=program_price_selection]", this).attr(
        "data-campus"
      ),
      campusIdSelected = $("input[name=program_price_selection]", this).attr(
        "data-campus-id"
      ),
      ...
      materialFee = $("input[name=program_price_selection]", this).attr(
        "data-material-fee"
      );
...
const printStartDateSelection = function () {
  ...
  (programName = $("input[name=program_selection]").val()),
    ...
    (mktPrice = parseFloat(
      $("input[name=program_schedule_price_selection]").val()
    ));
  if (mktPrice != 0) {
    price = mktPrice;
  } else {
    ...
    .done(function (data) {
      ...
      for (let i = 0; i < dataObject.length; i++) {
        scheduleListArray.push({
          ...
          schedule: dataObject[i].values["schedule"].name
            .replace(/\s/g, "")
            .replace("Weeks", ""),
        });
      }
...
const getOtherProgramFees = function () {
  let campus = $("input[name=campus_selection]").val(),
    ...
    queryParam =
      "&campus_new__in=" +
      campusId +
      "&category__eq=Other Program Fees&enable__eq=1&sort=-price_current",
    api_url = apiUrl + tableId + "/rows?portalId=" + portalId + queryParam;
  api_url = encodeURI(api_url);
  let feeArray = [];
...
const printQuotesHTML = function () {
  ...
  if (dataDisableResidenceCurrency === "true") {
    $(".x-convert-container .btn[data-destination-currency]")
      .addClass("active")
      .addClass("btn-orange-outline")
      .removeClass("btn-black-outline");
    ...
    for (let i = 0; i < quoteObj.quote_array.length; i++) {
      ...
      if (quoteObj.quote_array[i].quote_enabled) {
        ...
        quoteHTML =
          ...
          promoCodeHtml +
          "</div><div class='quote-box total-price-container'>" +
          totalPriceHtml +
          "</div>" +
          accommodationNoteHtml +
          priceNoteHtml +
